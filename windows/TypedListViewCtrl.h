			...
			if (di && di->item.iSubItem >= 0)
			{
				if (di->item.lParam)
				{
					if (di->item.mask & LVIF_TEXT)
					{
						...
						// ≈сли не делать SetText - глючит инфа в подсказке на юзере
						{
	...
	public:
		...
		void removeGroupedItem(T* item, bool removeFromMemory = true)
		{
			...
			else
			{
				...
				const auto l_id = deleteItem(item); // TODO - разобратьс€ почему тут не удал€ет.
#ifdef _DEBUG
				if (l_id < 0)
				{
	...
	public:
		...
		void removeGroupedItem(T* item, bool removeFromMemory = true)
		{
			...
			else
			{
				...
				deleteItem(item); // TODO - разобратьс€ почему тут не удал€ет.
