...
{
	if (!m_isUpdateColumnsInfoProcessed) // Àïäåéò êîëîíîê äåëàåì òîëüêî îäèí ðàç ïðè ïåðâîé àêòèâàöèè ò.ê. ListItem íå ðàçðóøàåòñÿ
	{
		...
		// m_ctrlUsers->setSortColumn(-1); // TODO - íàó÷èòñÿ ñîðòèðîâàòü ïîñëå àêòèâàöèè ôðåéìà à íå â íà÷àëå
...
void HubFrame::createMessagePanel()
{
	...
	if (!isClosedOrShutdown())
	{
		if (m_ctrlFilter == nullptr && ClientManager::isStartup() == false)
		{
			...
			updateColumnsInfo(fhe); // Íàñòðîèì êîëîíêè ñïèñêà þçåðîâ
			...
			if (m_ActivateCounter == 1)
			{
				...
				updateSplitterPosition(fhe); // Îáíîâèì ñïëèòåð
			}
		...
		if (l_is_need_update)
		{
			...
			HubModeChange();
#endif
			UpdateLayout(TRUE); // TODO - ñêîíñòðóèðîâàòü ñòàòóñ îòäåëüíûì ìåòîäîì
			restoreStatusFromCache(); // Âîññòàíàâëèâàòü ñòàòóñ íóæíî ïîñëå UpdateLayout
		}
	}
}
void HubFrame::destroyMessagePanel(bool p_is_destroy)
{
	...
	if (m_ctrlFilter)
	{
		...
		if (!l_is_shutdown && m_closed == false && m_before_close == false)
		        ...
		        */
		safe_destroy_window(m_tooltip_hubframe); // èñïîëüçóåò m_ctrlSwitchPanels è m_ctrlShowUsers è m_ctrlShowMode
...
void HubFrame::onBeforeActiveTab(HWND aWnd)
{
	...
	if (ClientManager::isStartup() == false)
	{
		...
		for (auto i = g_frames.cbegin(); i != g_frames.cend(); ++i) // TODO ïîìíèòü ïîñëåäíèé è íå ïåðåáèðàòü âñå äëÿ ðàçðóøåíèÿ.
		{
	...
	if (!ClientManager::isBeforeShutdown())
	{
		if (ClientManager::isStartup() == false)
		{
			if (m_ctrlStatus)
			{
				...
				// TODO ïîäîáðàòü áîëåå ëåãêóþ êîìàíäó. áåç ýòîé ïðîïàäàþ èêîíêè â ñòàòóñå.
...
FavoriteHubEntry* HubFrame::addAsFavorite(const FavoriteManager::AutoStartType p_autoconnect/* = FavoriteManager::NOT_CHANGE*/)
{
	...
	dcassert(existingHub); // Ôóíêöèÿ âîçâðàùàåò èíîãäà nullptr https://crash-server.com/DumpGroup.aspx?ClientID=guest&DumpGroupID=39084
	return existingHub;
}
...
bool HubFrame::updateUser(const OnlineUserPtr& p_ou, const int p_index_column)
{
	...
	if (ui == nullptr) // Þçåð ñêðûò èëè õàá
	{
		return false;
	}
	else // Þçåðà íàøëè è íóæíî îáíîâèòü
	{
		if (ui->isHidden())
		{
			if (m_showUsers)
		...
		else
		{
			if (m_showUsers)
			{
				...
				// [!] TODO if (m_client->is_all_my_info_loaded()) // TODO íåëüçÿ òóò îòêëþ÷àòü èíà÷å ãëþ÷èò îáíîâëåíèÿ ñâîåãî íèêà åñëè õàá PtoX ó ñàìîãî ñåáÿ øàðà = 0
				if (m_ctrlUsers)
				{
					m_needsResort |= ui->is_update(m_ctrlUsers->getSortColumn());
					...
					if (pos != -1)
					{
						// Äëÿ íåâèäèìûõ þçåðîâ òîæå íóæíî àïäåéòèòü êîëîíêè (Øàðà/ñîîáùåíèÿ è ò.ä.
						// if (pos >= l_top_index && pos <= l_top_index + m_ctrlUsers->GetCountPerPage()) // TODO m_ctrlUsers->GetCountPerPage() çàêåøèðîâàòü?
						{
#if 0
#ifdef _DEBUG
							...
							//if (Text::toT(ui->getUser()->getLastNick()) == _T("Òàëèñìàí"))
							//{
							//  LogManager::message("Òàëèñìàí");
							//}
					...
					{
						// Íå íàøëè ýëåìåíò - îí ñïðÿòàí ôèëüòðîì
						// dcassert(0);
					}
				}
			}
...
void HubFrame::addStatus(const tstring& aLine, const bool bInChat /*= true*/, const bool bHistory /*= true*/, const CHARFORMAT2& cf /*= WinUtil::m_ChatTextSystem*/)
{
	...
	}
	{
		if (!isConnected() && !m_last_hub_message.empty())
		{
			...
			{
				// 1. Ñîõðàíÿåì ïîñëåäíåå ñîîáùåíèå êîòîðîå ïðèõîäèò îò áîòà èëè õàáà
				// 2. Åñëè ïîñëå ýòîãî ïîëó÷àåì äèñêîííåêò - ïåðåõîäèì ê ïàðñèíãó ïîñëåäíåé ìåññàãè õàáà.
				// 3. Èùåì óïîìèíàíèå òåêóùåãî IP â ñòðîêå
				// 4. Åñëè íàøëè - èùåì ñëåäóþùèé IP è èñïîëüçóåì åãî çíà÷åíèå ïðè ñëåäóþùåì ïîäêëþ÷åíèè ê ýòîìó õàáó
				
				// RusHub "Â ïîèñêîâîì çàïðîñå âû îòñûëàåòå íåâåðíûé IP àäðåñ: 127.0.0.1, âàø ðåàëüíûé IP: 172.23.17.18."
...
#if 0 // Íåëüçÿ âêëþ÷èò - ìèãàþò ÷àñû
		...
		case WM_SPEAKER_UPDATE_USER_JOIN:
		{
			...
			if (!ClientManager::isBeforeShutdown())
			{
				if (updateUser(*l_ou))
				{
					...
					const bool isFavorite = !FavoriteManager::isNoFavUserOrUserBanUpload(user); // [!] TODO: â ÿäðî!
					if (isFavorite)
					{
	...
	if (isConnected())
	{
		if (updateUser(p_ou, -1))
		{
			...
			if (m_client->is_all_my_info_loaded())
			{
				...
				const bool isFavorite = !FavoriteManager::isNoFavUserOrUserBanUpload(p_ou->getUser()); // [!] TODO: â ÿäðî!
				if (isFavorite)
				{
					PLAY_SOUND(SOUND_FAVUSER);
					SHOW_POPUP(POPUP_FAVORITE_CONNECTED, id.getNickT() + _T(" - ") + Text::toT(m_client->getHubName()), TSTRING(FAVUSER_ONLINE));
		...
		if (!ClientManager::isBeforeShutdown())
		{
			switch (i->first)
			{
				...
				case UPADTE_COLUMN_SHARE:
				{
					...
					m_needsUpdateStats |= updateUser(u.m_ou, COLUMN_SHARED); // TODO  ïåðåäàòü âòîðîé ïàðàìåòð
				}
				break;
#endif
				...
				case STATS:
				{
					...
					if (m_client && m_client->is_all_my_info_loaded() == true)
					{
						...
						if (m_needsResort && m_ctrlUsers && m_ctrlStatus && l_count_item > m_last_count_resort && !MainFrame::isAppMinimized())
						{
							...
							m_needsResort = false;
							m_ctrlUsers->resort(); // óáðàí ðåñîðò åñëè îêíî íå àêòèâíîå!
#ifdef _DEBUG
...
void HubFrame::updateWindowText()
{
	...
	if (isClosedOrShutdown())
		return;
	if (ClientManager::isStartup() == false) // Ïîêà êîíñòðóèðóåìñÿ íå íóæíî àïäåéòèòü òåêñò
	{
		if (m_is_window_text_update)
		{
			// TODO - îãðàíè÷èòü ðàçìåð òåêñòà
...
void HubFrame::UpdateLayout(BOOL bResizeBars /* = TRUE */)
{
	...
	if (m_ctrlStatus)
	{
		if (m_ctrlStatus->IsWindow() && m_ctrlLastLinesToolTip->IsWindow())
		{
			...
			int HubPic = 0;
			int l_hubIcoSize = 0;   // Øèðèíà èêîíêè ðåæèìà
#ifdef SCALOLAZ_HUB_MODE
			if (BOOLSETTING(ENABLE_HUBMODE_PIC))
			{
				l_hubIcoSize = 22;  // Øèðèíà èêîíêè ðåæèìà ( 16 px )
				HubPic += l_hubIcoSize;
			}
#endif
#ifdef SCALOLAZ_HUB_SWITCH_BTN
	...
	if (m_msgPanel)
	{
		...
		{
			TuneSplitterPanes();
			if (!m_showUsers) // Åñëè ñïèñîê ïîëüçîâàòåëåé íå îòîáðàæàåòñÿ.
			{
				if (GetSinglePaneMode() == SPLIT_PANE_NONE) // Åñëè íèêàêàÿ ñòîðîíà íå ñêðûòà.
				{
#ifdef SCALOLAZ_HUB_SWITCH_BTN
			...
			else // Åñëè ñïèñîê ïîëüçîâàòåëåé îòîáðàæàåòñÿ.
			{
				if (GetSinglePaneMode() != SPLIT_PANE_NONE) // Åñëè êàêàÿ-òî ñòîðîíà ñêðûòà.
					SetSinglePaneMode(SPLIT_PANE_NONE); // Íèêàêàÿ ñòîðîíà íå ñêðûòà.
			}
			SetSplitterRect(rc);
		}
...
void HubFrame::TuneSplitterPanes()
{
	...
	if (m_ctrlUsers && ctrlClient.IsWindow())
	{
		...
		{
			SetSplitterPanes(ctrlClient.m_hWnd, m_ctrlUsers->m_hWnd, false); // ×àò, ñïèñîê ïîëüçîâàòåëåé.
		}
		else // Åñëè ñïèñîê ïîëüçîâàòåëåé ñëåâà.
		{
			SetSplitterPanes(m_ctrlUsers->m_hWnd, ctrlClient.m_hWnd, false); // Ñïèñîê ïîëüçîâàòåëåé, ÷àò.
		}
#else
	...
	if (fhe)
	{
		...
		}
		{
			...
			if (g_frames.size() == 1 ||
				...
				// Ñîõðàíÿåì òîëüêî íà ïîñëåäíåé èòåðàöèè,
				// èëè êîãäà íå çàêðûâàåì/çàïóñêàåì ïðèëîæåíèå.
				// èëè åñëè ïîìåíÿëèñü ðàçìåðû êîëîíîê
			{
...
void HubFrame::clearUserList()
{
	...
	if (m_ctrlUsers)
	{
		CLockRedraw<> l_lock_draw(*m_ctrlUsers); // TODO ýòî íóæíî èëè îïóñòèòü íèæå?
...
void HubFrame::addLine(const Identity& p_from, const bool bMyMess, const bool bThirdPerson, const tstring& aLine, unsigned p_max_smiles, const CHARFORMAT2& cf /*= WinUtil::m_ChatTextGeneral*/)
{
	...
	if (BOOLSETTING(LOG_MAIN_CHAT))
	{
		...
		// TODO crash "<-FTTBkhv-> Running Verlihub 1.0.0 build Fri Mar 30 2012 ][ Runtime: 5 äí. 15 ÷àñ. ][ User count: 533"
		if (!extra.empty())
...
LRESULT HubFrame::onFileReconnect(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	{
		MessageBox(CTSTRING(ENTER_NICK), getFlylinkDCAppCaptionWithVersionT().c_str(), MB_ICONSTOP | MB_OK);// TODO Äîáàâèòü àäðåñ õàáà â ñîîáùåíèå
		return 0;
	}
	m_client->reconnect();
	return 0;
...
void HubFrame::closeAll(size_t thershold)
{
	if (thershold == 0)
	{
		// Óñêîðèì çàêðûòèå âñåõ õàáîâ
		FavoriteManager::getInstance()->prepareClose();
		ClientManager::getInstance()->prepareClose();
		// ClientManager::getInstance()->prepareClose(); // Îòïèøåìñÿ îò ïîäïèñîê êëèåíòà
		// SearchManager::getInstance()->prepareClose(); // Îòïèøåìñÿ îò ïîäïèñîê ïîèñêà
	}
	{
		CFlyLock(g_frames_cs);
		for (auto i = g_frames.cbegin(); i != g_frames.cend(); ++i)
...
void HubFrame::timer_process_all()
{
	...
	for (auto i = g_frames.cbegin(); i != g_frames.cend(); ++i)
	{
		if (!i->second->isClosedOrShutdown())
		{
			i->second->timer_process_internal(); // TODO ïðîêèíóòü ôëàã âèäèìîñòè ÷òîáû íå îáíâëÿòü ñòàòóñ
		}
	}
}
#endif
		        ...
		        && !isClosedOrShutdown())
		{
			   ...
			   )
			{
				...
				if (!m_is_first_goto_end)
				{
					...
					ctrlClient.GoToEnd(true); // Ïîêà íå ïàøåò è íå ïîÿâëÿåòñÿ ñêðîëëåð
				}
#endif
			}
		}
...
// TODO
	...
	*/
	...
	// ßâíî çâàòü addStatus íåëüçÿ - âåøàåìñÿ ïî÷åìó-òî
...
void HubFrame::onTimerHubUpdated()
{
	...
	if (m_client && m_hub_name_update_count)
	{
		...
		if (!BOOLSETTING(SHOW_FULL_HUB_INFO_ON_TAB)) // Íå ïîêàçûâàåì èíôó ñ õàáà
		{
...
LRESULT HubFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
{
	...
	// Ïîñëåäîâàòåëüíîñòü ñîáûòèé ïðè îòðèñîâêå
	CRect rc;
...
{
	// Äàííàÿ ôóíêöèÿ ëîìàåò ìåíþ - http://youtu.be/GaWw-S4ZYJA
	// Ïðè÷èíó ïîêà íå çíàþ - åñòü êðàøè https://crash-server.com/Problem.aspx?ClientID=guest&ProblemID=27075
	// L: ðåòóðí óáðàë, âåäü íå ïîìîãëî æå! return; // http
	/*
	r502-beta94-x64 build 14474
	êîñÿê ñ ïêì ïîñëå alt+d íè êóäà íå äåëñÿ
	L: åñòü çíà÷èòåëüíàÿ âåðîÿòíîñòü òîãî, ÷òî ïîñëå ìîåãî ðåôàêòîðèíãà ïðîáëåìû èñ÷åçíóò, ïðîøó îòïèñàòüñÿ.
	*/
		...
		for (auto f = g_frames.cbegin(); f != g_frames.cend(); ++f)
		{
			...
			for (auto i = frame->m_userMap.cbegin(); i != frame->m_userMap.cend(); ++i) // TODO https://crash-server.com/Problem.aspx?ClientID=guest&ProblemID=28097
			{
				...
				if ((p_param.m_bytesShared && l_id.getBytesShared() == p_param.m_bytesShared) ||
				        ...
				        (!p_param.m_ip.empty() && p_param.m_ip == l_cur_ip)) // .getIpAsString() - íåëüçÿ îíà çàáèðàåò àäðåñ èç áàçû è òîðìîçèò
				{
