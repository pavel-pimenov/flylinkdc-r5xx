...
class ProvidersPage : public CPropertyPage<IDD_PROVIDERS_PAGE>, public PropPage
{
	public:
		...
		ProvidersPage(SettingsManager *s) : PropPage(s)/*, bInited(false)*/
		{
			SetTitle(CTSTRING(SETTINGS_PROVIDERS));
...
{
	public:
		ProvidersPage(SettingsManager *s) : EmptyPage(s, TSTRING(SETTINGS_PROVIDERS))
		{
		}
};
