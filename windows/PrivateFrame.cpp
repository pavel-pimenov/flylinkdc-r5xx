                              ...
                              const bool bThirdPerson, const bool notOpenNewWindow /*= false*/)   // !SMT!-S
{
	...
	if (i == g_pm_frames.end())
	{
		if (notOpenNewWindow || g_pm_frames.size() > MAX_PM_FRAMES)
			return false; // !SMT!-S
		/*
		15:50:13 <HackFresse> есть возможность получить количество открытых личек с одного хаба?  а потом просто условие "если с хаба пришла личка, и открытых личек с этого хаба уже 20 --направл€ем личку
		                      в чат хаба"
		15:52:13 <FlylinkDC-dev-linux> 20 это может много?
		15:52:23 <FlylinkDC-dev-linux> ты же сам писал что никто не общаетс€
		15:52:55 <HackFresse> нужна статистика, которой у мен€ нет
		...
		15:56:49 <HackFresse> все сразу флудить начнут вр€д-ли, а штук 5 по 20 личек -- всего 100 окон, + какие-то открытые без флуда, клиент виснуть не должен (если 200 окон отрабатывает нормально)
		*/
...
LRESULT PrivateFrame::onTabContextMenu(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
{
	...
	tabMenu.AppendMenu(MF_STRING, IDC_CLOSE_ALL_OFFLINE_PM, CTSTRING(MENU_CLOSE_ALL_OFFLINE_PM)); // [+] InfinitySky. «акрыть все оффлайн лички.
	tabMenu.AppendMenu(MF_STRING, IDC_CLOSE_ALL_PM, CTSTRING(MENU_CLOSE_ALL_PM)); // [+] InfinitySky. «акрыть все лички.
...
void PrivateFrame::updateTitle()
{
	...
	{
		banIcon = FavoriteManager::hasUploadBan(l_ul) || FavoriteManager::hasIgnorePM(l_flags); // TODO - переписать на получени€ иконки
	}
