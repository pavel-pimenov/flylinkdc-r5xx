...
void ToWidePath(const std::string& value, std::wstring& target) {
	...
	target.resize(value.length() + 1);
	while ((size = MultiByteToWideChar(CP_UTF8, 0, value.c_str(), value.length(), &target[0], (int)target.length())) == 0)
	{
...
void ToNarrowPath(const std::wstring& value, std::string& target) {
	// [!] IRainamn fix.
	std::wstring::size_type size = 0;
	target.resize(value.length() * 2);
	while ((size = WideCharToMultiByte(CP_UTF8, 0, value.c_str(), value.length(), &target[0], target.length(), NULL, NULL)) == 0)
	{
		if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
		{
			target.resize(target.size() * 2);
...
#ifndef _UNICODE
std::string GetCurrentDir() // [!] IRainamn fix.
{
    CHAR path[MAX_PATH];
...
#else
std::wstring GetCurrentDir() // [!] IRainamn fix.
{
    WCHAR path[MAX_PATH];
...
std::string& ModifyPath(std::string& path)
{
	// [+] IRainamn fix.
#ifndef _UNICODE
	static const std::string g_currentDir = GetCurrentDir();
#else
	static std::string g_currentDir;
	if (g_currentDir.empty())
	{
		ToNarrowPath(GetCurrentDir(), g_currentDir);
	}
#endif
	// [~] IRainamn fix.

    if((path[0] == '/' && path[1] != '/') || (path[0] == '\\' && path[1] != '\\')){
        path = g_currentDir + path;
    }
...
std::wstring& ModifyPath(std::wstring& path)
{
	// [+] IRainamn fix.
#ifndef _UNICODE
	static std::wstring g_currentDirW;
	if (g_currentDirW.empty())
	{
		ToWidePath(GetCurrentDir(), g_currentDirW);
	}
#else
	static const std::wstring g_currentDirW = GetCurrentDir();
#endif
	// [~] IRainamn fix.

    if((path[0] == L'/' && path[1] != L'/') || (path[0] == L'\\' && path[1] != L'\\')){
        path = g_currentDirW + path;
    }
...
bool Win32MapFile::_MapNewRegion()
{
    ...
    _base_handle = CreateFileMapping( // [!] IRainman fix: use unicode function if possible.
        _hFile,
        NULL,
        PAGE_READWRITE,
        0,
...
static Status CreateDirInner( const std::string& dirname )
{
	...
	// [+] IRainman fix.
    DWORD attr = ::GetFileAttributes(dirNameW.c_str()); // [!] IRainman fix.
    if (attr == INVALID_FILE_ATTRIBUTES) { // doesn't exist:
      const std::size_t slash = dirname.find_last_of('\\');
      if (slash != std::string::npos){
	sRet = CreateDirInner(dirname.substr(0, slash));
	if (!sRet.ok()) return sRet;
      }
      const BOOL result = ::CreateDirectory(dirNameW.c_str(), NULL); // [!] IRainman fix.
      if (result == FALSE) {
