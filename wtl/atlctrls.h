...
// Attributes
	...
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return (UINT)::GetWindowLongPtr(m_hWnd, GWL_STYLE) & 0xFFFF;
	}
...
// Attributes
	...
	BOOL GetItemText(HTREEITEM hItem, BSTR& bstrText) const
	{
		...
		for(int nLen = 256; ; nLen *= 2)
		{
			...
			if(!bRet || (wcslen(item.pszText) < nLen - 1)) // V303 The function 'lstrlen' is deprecated in the Win64 system. It is safer to use the 'wcslen' function. atlctrls.h 4060
				break;
			delete [] lpstrText;
			lpstrText = NULL;
		}
...
// Operations
	...
	BOOL GetItemText(int nIndex, BSTR& bstrText) const
	{
		...
		for(int nLen = 256; ; nLen *= 2)
		{
			...
			if(!bRet || (wcslen(cbex.pszText) < nLen - 1)) // V303 The function 'lstrlen' is deprecated in the Win64 system. It is safer to use the 'wcslen' function. atlctrls.h 8851
				break;
			delete [] lpstrText;
			lpstrText = NULL;
		}
