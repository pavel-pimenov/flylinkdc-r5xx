...
void File_AvsV::Data_Parse()
{
    ...
    switch (Element_Code)
    {
        ...
        default:
            if (Element_Code>=0x00 && Element_Code<=0xAF) //[!]PVS-Studio	V547	Expression 'Element_Code >= 0x00' is always true. Unsigned type value is always >= 0.	MediaInfoLib	file_avsv.cpp	418	False	
              slice();
            else
            {
