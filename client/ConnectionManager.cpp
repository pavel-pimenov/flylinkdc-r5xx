	...
	{
		{
			...
			else
			{
				...
				{
					m_checkIdle.push_back(aUser); // TODO - Ëîê?
				}
			}
#endif
		}
...
void ConnectionManager::putCQI_L(ConnectionQueueItemPtr& cqi)
{
	...
	// Âåøàåìñÿ ïðè àêòèâíîé çàêà÷êå cqi->getUser()->flushRatio();
#endif
...
bool ConnectionManager::getCipherNameAndIP(UserConnection* p_conn, string& p_chiper_name, string& p_ip)
{
	...
	if (l_conn != g_userConnections.end())
	{
		...
		p_ip = p_conn->getRemoteIp(); // TODO - ïåðåâåñòè íà boost?
		return true;
	}
	return false;
}
...
void ConnectionManager::on(TimerManagerListener::Second, uint64_t aTick) noexcept
{
			...
			if (cqi->getState() != ConnectionQueueItem::ACTIVE) // crash - https://www.crash-server.com/Problem.aspx?ClientID=guest&ProblemID=44111
			{
				                                   ...
				                                   cqi->getLastAttempt() + l_count_sec * 1000 * std::max(1, l_count_error) < aTick))
				{
					...
					if (cqi->getState() == ConnectionQueueItem::WAITING)
					{
						if (startDown)
						{
							...
							{
								cqi->m_count_waiting++;
								cqi->m_is_force_passive = cqi->m_is_active_client ? (cqi->m_count_waiting > 1) : false; // Äåëàåì âòîðóþ ïîïûòêó ïîäêëþ÷åíèÿ â ïàññèâêå ?
							}
							else
							{
				...
				else if (cqi->getState() == ConnectionQueueItem::CONNECTING && cqi->getLastAttempt() + l_count_sec_connecting * 1000 < aTick)
				{
					...
					if (cqi->m_count_waiting > 2)
					{
						...
						cqi->setState(ConnectionQueueItem::WAITING);
						// TODO - óäàëåíèå ïîêà íå äåëàåì - íóæíî ïîòåñòèðîâàòü ëó÷øå
						// l_removed.push_back(cqi);
					}
					else
#endif
	...
	// TODO - íå çâàòü äëÿ òåõ ó êîãî îøèáêà çàãðóçêè
...
void ConnectionManager::cleanupIpFlood(const uint64_t p_tick)
{
	...
	{
		// Åñëè êîííåêòîâ ñîâåðøåíî ìåíüøå ÷åì ïðåäåë â òå÷åíèè ìèíóòû - óáèðàåì àäðåñ èç òàáëèöû - ñ íèì âñå õîðîøî!
		...
		// Åñëè êîííåêòîâ ñîâåðøåíî ìíîãî è IP íàõîäèòñÿ â áàíå, íî óæå ïðîøëî âðåìÿ áîëüøå ÷åì 5 Ìèíóò(ïî óìîë÷àíèþ)
		// Òàêæå óáèðàåì çàïèñü èç òàáëèöû áëîêèðîâêè
		...
		{
			string l_type;
			if (j->first.m_ip.is_unspecified()) // Åñëè íåò âòîðîãî IP òî ýòî êîìàíàäà  ConnectToMe
			{
...
void ConnectionManager::cleanupDuplicateSearchFile(const uint64_t p_tick)
{
	...
	for (auto j = g_duplicate_search_file.cbegin(); j != g_duplicate_search_file.cend();)
	{
		if ((p_tick - j->second.m_first_tick) > 1000 * CFlyServerConfig::g_max_unique_file_search)
		{
			...
			if (j->second.m_count_connect > 1) // Ñîáûòèå âîçíèêàëî áîëüøå îäíîãî ðàçà - ëîãèðóåì?
			{
...
#else
		if ((l_connection->getLastActivity(false) + 60 * 1000) < aTick) // Çà÷åì òàê ìíîãî ìèíóò âèñåòü?
#endif
		{
	...
	m_is_secure(p_is_secure)
{
	...
	m_server_ip   = p_server_ip; // â AirDC++ è äóðãèõ ýòîãî óæå íåò
...
void ConnectionManager::accept(const Socket& sock, bool secure, Server* p_server) noexcept
{
	...
	{
		/*if (false  // TODO - óçíàòü ïî÷åìó òóò òàêîé çàòûêîí îñòàâëåí â îðèãèíàëüíîì dc++
	...
	catch (const Exception& e)
	{
		...
		// Îáðàáîòêà òåñòà ïîðòà TLS
		if (secure && p_server)
		{
			const auto l_remote_port = p_server->getServerPort();
			if (l_remote_port == SETTING(TLS_PORT)) // TODO ïðîâåðèòü òóò IP âíåøíåãî ñåðâåðà - îí äîëæåí ñîâïàäàòü ñ test.dchub.net
			{
				SettingsManager::g_TestTLSLevel = true; // Ïðîâåðüòü ìàãè÷åñêîå ÷èñëî òóò íå çíàþ êàê - ñ÷èòàåì åñëè ïðèøåë accept íà ýòîò ïîðò, òî îí îòêðûò
...
bool ConnectionManager::checkDuplicateSearchFile(const string& p_search_command)
{
	...
	if (l_key_pos != string::npos && l_key_pos)
	{
		...
		if (l_result.second == false) // Ýëåìåíò óæå ñóùåñòâóåò - ïðîâåðèì åãî ñ÷åò÷èê è ñòàðîñòü.
		{
			l_cur_value.m_last_tick = l_tick;
			if (l_tick - l_cur_value.m_first_tick > 1000 * CFlyServerConfig::g_max_unique_file_search)
			{
				// Òóò ìîæíî ñðàçó ñòåðåòü ýëåìåíò óñòàðåâøèé
				return false;
			}
...
bool ConnectionManager::checkIpFlood(const string& aIPServer, uint16_t aPort, const boost::asio::ip::address_v4& p_ip_hub, const string& p_userInfo, const string& p_HubInfo)
{
	...
	}
	{
		...
		// dcassert(!ec); // TODO - òóò áûâàåò è Host
		...
		{
			// Ýëåìåíò óæå ñóùåñòâóåò
			l_cur_value.m_last_tick = l_tick;   // Êîððåêòèðóåì âðåìÿ ïîñëåäíåé àêòèâíîñòè.
			l_cur_value.m_ports.insert(aPort);  // Ñîõðàíèì ïîñëåäíèé ïîðò
			if (l_cur_value.m_count_connect == CFlyServerConfig::g_max_ddos_connect_to_me) // Ïðåâûñèëè êîë-âî êîííåêòîâ ïî îäíîìó IP
			{
				static uint16_t g_block_id = 0;
				l_cur_value.m_block_id = ++g_block_id;
				if (BOOLSETTING(LOG_DDOS_TRACE))
				{
					...
					{
						LogManager::ddos_message("  Detail BlockID=" + Util::toString(l_cur_value.m_block_id) + " " + k->first + " Count:" + Util::toString(k->second)); // TODO - ñäàòü äóáëèêàòû + ïîêàçàòü êîë-âî
					}
				}
			...
			if (l_cur_value.m_count_connect >= CFlyServerConfig::g_max_ddos_connect_to_me)
			{
				...
				{
					return true; // Ëî÷èì ýòîò êîííåêò äî íàñòóïëåíèÿ àìíèñòèè. TODO - ïðîâåðèòü ýòó ÷àñòü âíèìàòåëüåé
					// â ñëåä ÷àñòè ôèêñà - ïðîâîäèòü àíàëèç ïðîòîêîëà è êîííåêòû íà ïîðòû ëî÷èòü íà âå÷íî.
				}
			}
		}
	}
	{
		...
		for (auto j = g_userConnections.cbegin(); j != g_userConnections.cend(); ++j)
		{
			...
			if (uc.socket == nullptr || !uc.socket->hasSocket())
				continue;
			if (uc.getPort() == aPort && uc.getRemoteIp() == aIPServer) // TODO - íå ïîääåðæèâàåòñÿ DNS
			{
				if (++count >= 5)
				{
                                    ...
                                    bool secure)
{
	...
	uc->setUserConnectionToken(aNick); // Òîêåí = íèê?
	uc->setHubUrl(hubUrl);
...
{
	safe_delete(server); // TODO Çîâåòñÿ ÷àùå ÷åì íóæíî.
...
void ConnectionManager::on(UserConnectionListener::MyNick, UserConnection* aSource, const string& aNick) noexcept
{
	...
	if (aSource->isSet(UserConnection::FLAG_INCOMING))
	{
		...
		aSource->setHubUrl(i.m_HubUrl); // TODO - òóò þçåðà ïî÷åìó-òî åùå íåò
	...
	if (aSource->isSet(UserConnection::FLAG_ASSOCIATED))
	{
		...
		if (l_is_download)
		{
			...
			else
			{
				...
				//!!! putCQI_L(cqi); íå äåëàåì îòêëþ÷åíèå - òåðÿåì äîêà÷êó https://github.com/pavel-pimenov/flylinkdc-r5xx/issues/1679
...
#ifdef FLYLINKDC_USE_LASTIP_AND_USER_RATIO
	// Ñáðàñûâàåì ðåéòèíã â áàçó ïîêà íå íàøëè ïðè÷èíó ïî÷åìó òóò îñòàþòñÿ çàïèñè.
	{
		{
			CFlyReadLock(*g_csDownloads);
			for (auto i = g_downloads.cbegin(); i != g_downloads.cend(); ++i)
...
{
	dcassert(p_conn->getUser());
	if (p_conn->getUser()) // 44 ïàäåíèÿ https://www.crash-server.com/Problem.aspx?ClientID=guest&ProblemID=48388
	{
