...
static void gf_trace_callback(void* p_udp, const char* p_sql)
{
	if (g_DisableSQLtrace == 0)
	{
		if (BOOLSETTING(LOG_SQLITE_TRACE) || g_EnableSQLtrace)
		{
			...
			LOG_FORCE_FILE(TRACE_SQLITE, params); // Âñåãäà â ôàéë
		}
	}
}
//========================================================================================================
...
bool CFlylinkDBManager::safeAlter(const char* p_sql, bool p_is_all_log /*= false */)
{
	...
	{
		if (p_is_all_log == true || e.getError().find("duplicate column name:") == string::npos) // Ëîãèðóåì òîëüêî íåèçâåñòíûå îøèáêè
		{
...
void CFlylinkDBManager::errorDB(const string& p_txt)
{
	...
	const tstring l_footer = _T("Åñëè ýòî íå ïîìîæåò ïèøèòå ïîäðîáíîñòè íà pavel.pimenov@gmail.com èëè ppa74@ya.ru\r\n")
	                         _T("ïîìîãó ðàçîáðàòüñÿ ñ îøèáêîé è èñïðàâèòü êîä ôëàéëèíêà òàê,\r\n")
	                         _T("÷òîáû àíàëîãè÷íîé ïðîáëåìû íå âîçíèêàëî ó äðóãèõ ïîëüçîâàòåëåé.\r\n")
	                         _T("ñêîïèðîâàòü ñîîáùåíèå ñ ýòîãî îêíà ìîæíî íàæàòèåì êëàâèø Ctrl+C\r\n")
	                         _T("Ñïàñèáî çà ïîíèìàíèå.\r\n\r\n");
	...
	if (l_is_db_ro)
	{
		...
		l_message += l_rnrn;
		l_russian_error +=
		    _T("Áàçà äàííûõ íàõîäèòñÿ â êàòàëîãå çàùèùåííîì îò çàïèñè, èëè ïîä çàùèòîé UAC\r\n")
		    _T("Âàðèàíòû èñïðàâëåíèÿ ñèòóàöèè:\r\n")
		    _T(" 1. Óñòàíîâèòå ïðîãðàììó â êàòàëîã îòëè÷íûé îò C:\\Programm Files*\r\n")
		    _T("    íàïðèìåð D:\\FlylinkDC\r\n")
		    _T(" 2. Óäàëèòå êàòàëîã C:\\Programm Files\\FlylinkDC++\\Settings\r\n")
		    _T("    ïîñëå ïåðåçàïóñêà ïðîãðàììà ñîçäàñò íàñòðîéêè â ñâîåì ïðîôèëå\r\n")
		    _T("    ãäå UAC íå áóäåò ìåøàòü ðàáîòå\r\n");
		l_russian_error += l_footer;
		l_is_force_exit = true;
	}
	if (l_is_db_malformed || l_is_db_error_open)   // TODO - ïîïîëíÿòü ýòè îøèáêè èç àâòîàïäåéòà?
	{
		l_message += l_rnrn;
		l_message += l_rnrn;
		...
		l_message += l_rnrn;
		l_russian_error +=
		    _T("Áàçà äàííûõ ðàçðóøåíà!\r\n")
		    _T("âîçìîæíî êîìïüþòåð âûêëþ÷èëè íå êîððåêòíî\r\n")
		    _T("èëè îí çàâèñ ñ ñèíèì ýêðàíîì\r\n\r\n")
		    _T("Íå âîëíóéòåñü... Äëÿ èñïðàâëåíèÿ ñèòóàöèè âûïîëíèòå ñëåäóþùåå:\r\n")
		    _T(" 1. Çàêðîéòå FlylinkDC++\r\n")
		    _T(" 2. Óäàëèòå óêàçàííûå âûøå ôàéëû (*.sqlite)\r\n")
		    _T("    (åñëè æåëàåòå ïðîâåñòè äåòàëüíûé àíàëèç ïðè÷èí ðàçðóøåíèÿ\r\n")
		    _T("    ïðåäâàðèòåëüíî ñîõðàíèòå ôàéëû â äðóãîì ìåñòå\r\n")
		    _T("    óïàêóéòå èõ àðõèâàòîðîì è âûøëèòå íà ïî÷òó ppa74@ya.ru\r\n")
		    _T(" 4. Çàïóñòèòå FlylnkDC++ ïîâòîðíî\r\n")
		    _T(" 5. Ïðîãðàììà àâòîìàòè÷åñêè ñîçäàñò íîâûå âåðñèè ôàéëîâ,\r\n")
		    _T("    è â ôîíîâîì ðåæèìå âûïîëíèò ïîâòîðíîå õåøèðîâàíèå øàðû.\r\n");
		l_russian_error += l_footer;
		l_is_force_exit = true;
	}
	...
	if (p_txt.find("database or disk is full") != string::npos)
	{
		l_message += l_rnrn;
		l_message += l_rnrn;
		l_russian_error = _T("Ó âàñ ïåðåïîëíèëñÿ æåñòêèé äèñê!\r\n")
		                  _T("óäàëèòå ëèøíèå äàííûå è îñâîáîäèòå ìåñòî äëÿ ðàáîòû ïðèëîæåíèÿ!\r\n");
		l_russian_error += l_footer;
		l_is_force_exit = true;
	}
	Util::setRegistryValueString(FLYLINKDC_REGISTRY_SQLITE_ERROR, Text::toT(l_error));
	LogManager::message(p_txt, true); // Âñåãäà ëîãèðóåì â ôàéë (ò.ê. áàçà ìîæåò áûòü áèòîé)
	static int g_MessageBox = 0; // TODO - fix copy-paste
	{
		CFlyBusy l_busy(g_MessageBox);
		if (g_MessageBox <= 1)
	...
	if (l_is_force_exit)
	{
		...
		if (g_is_first == false)
		{
			...
			// exit(1); // exit íåëüÿ - ïàäàåì  https://drdump.com/Bug.aspx?ProblemID=261050
			//
		}
	}
	if (!l_is_send)
	{
		// TODO - ñêèíóòü îøèáêó â ôàéë è íå ãðóçèòü crash-server ëîãè÷åñêèìè îøèáêàìè
...
#endif
	m_count_json_stat = 1; // Ïåðâûé ðàç äóìàåì, ÷òî â òàáëèöå ÷òî-òî åñòü
	...
	try
	{
		...
		else
		{
			if (SetCurrentDirectory(Text::toT(Util::getConfigPath()).c_str()))
			{
				...
				// Òóò îáÿçàòåëüíî ïîëíûé ïóòü. èíà÷å ïðè ñìåíå ðàáî÷åãî êàòàëîãà levelDB íå ñîìæåò îòêðûòü áàçó.
...
//				g_TTHLevelDBSize = File::calcFilesSize(l_full_path_level_db, "\\*.*"); // TODO óáðàòü ñêàíèðîâàíèå âòîðîå
				/*
				#ifdef _DEBUG
		...
		m_flySQLiteDB.executenonquery(
		    "CREATE TABLE IF NOT EXISTS media_db.fly_media(" // id integer primary key autoincrement, TODO - id â ýòîé òàáëè÷êå íàì íå íóæíà...
		    "tth_id integer not null,"
		    "stream_type integer not null,"
		    "channel integer not null,"
		    "param text not null,"
		    "value text not null);");
		// TODO - ïîòåñòèòü êàê sqlite âûáèðàåò ïëàí
		...
		m_flySQLiteDB.executenonquery(
		    ...
		    "Target text not null,"  // óáðàë UNIQUE äëÿ èñêëþ÷åíèÿ ïåðåñòðîéêè èíäåêñà (óíèêàëüíîñòü ïîääåðæèâàåòñÿ ìàïîé)
		    "Size int64 not null,"
...
//     if (l_rev <= 381)
		...
		{
			// Ïåðåçàãðóçèì ëîêàöèè â îòäåëüíûé ôàéë áàçû
			// Äëÿ ýòîãî ñêèíåì ìåòêó âðåìåíè äëÿ ôàéëîâ äàííûõ ÷ òîáû ïðè ñëåäóþùåì çàïóñêå âûïîëíèëàñü ïåðåçàãðóçêà
			// è óäàëèì òàáëèöû â îñíîâíîé áàçå äàííûõ
		...
		*/
		...
		// Îíà íå èñïîëüçóþòñÿ â âåðñèÿõ r502 íî äëÿ îòêàòà íàçàä íóæíû
		...
		if (l_rev < 403 || l_rev == 500)
		{
			...
			{
				// ïîëó÷èëîñü äîáàâèòü êîëîíêó - çíà÷èò ñòàðòàíóëè ïåðâûé ðàç - óäàëèì âñå çàïèñè äëÿ ïåðåõåøà ñ ïîìîùüþ ëèáû
		...
		m_flySQLiteDB.executenonquery("DROP INDEX IF EXISTS user_db.iu_user_info;"); //ñòàðûé èíäåêñ áûë (nick,dic_hub)
		...
		// TODO - ñäåëàòü ïîçæå åñëè áóäåò òîðìîçèòü
		...
		{
			// Óäàëÿþ óæå íà óðîâíå êîíâåðòîðà ôàéëà.
		...
		// TODO - ãðîõíóòü ïîëÿ  fly_queue â è ïåðåòàùèòü áàçó â äðóãèå ôàéëû.
		
		/*
		{
		    // Êîíâåðòèì ip â áèíàðíûé ôîðìàò
		    ...
		    {
		        // Ãàñèì îøèáêè ÁÄ ïðè êîíâåðòàöèè
...
{
	if (p_delete_old_info) // TODO - íèêîãäà íå ðàáîòàåò ïî÷åìó-òî
	{
	...
	if (p_media.isMediaAttrExists())
	{
		...
		{
			if (i->m_is_delete == false) // Ýòî ïàðàìåòð âðåìåííûé è åãî ïèñàòü â áàçó íå íóæíî
				// TODO - ïåðåñìîòðåòü àëãîðèòì è ïðè ãåíåðàöèè ìàññèâà óäàëÿòü ëèøíèè çàïèñè.
				// óáðàâ ëèøíèé ôëàã-ìåìáåð â ExtItem
			{
...
bool CFlylinkDBManager::load_media_info(const TTHValue& p_tth, CFlyMediaInfo& p_media_info, bool p_only_inform)
{
	...
	try
	{
		...
		if (l_tth_id)
		{
			// ×èòàåì áàçîâóþ èíôó (TODO - åñëè åñòü â ïàìÿòè ïîïðîáîâàòü çàáðàòü èç íåå)
			...
			if (l_q_base.read()) // çàáèðàåì âñåãäà îäíó çàïèñü. (limit = 1) â øàðå ìîæåò áûòü íåñêîëüêî îäèíàêîâûõ ôàéëîâ â ðàçíûõ êàòàëîãàõ.
			{
				...
				// p_media_info.calcEscape(); // Òóò ýòî íå íóæíî
			}
			if (p_media_info.isMedia()) // Åñëè åñòü áàçîâàÿ èíôîðìàöèÿ - ïîïûòàåìñÿ çàáðàòü äîïîëíèòåëüíóþ.
			{
				if (!p_only_inform)
				{
...
void CFlylinkDBManager::convert_fly_hash_block_crate_unicque_tthL(CFlyLogFile& p_convert_log)
{
	...
	catch (const database_error& e)
	{
		...
		// Óäàëÿåì äóáëè! íî ÿ íå çíàþ îòêóäà îíè ìîãóò âçÿòüñÿ :(
		{
...
#define FLYLINKDC_USE_FAST_CONVERT
	...
	if (is_table_exists("fly_hash"))
	{
		...
		clean_fly_hash_blockL();
		{
			...
			{
				if (e.getError().find("UNIQUE ") != string::npos) // Âåðîÿòíîñòü ýòîãî î÷åíü ìàëåíüêàÿ..
				{
					CFlyServerJSON::pushError(1, e.getError()); // TODO __FILE__, __LINE__
					convert_fly_hash_block_internalL(); // Îáðàáîòàåì àïäåéò ïî îäíîé çàïèñè TODO - åñëè íå âîçíèêíåò îøèáîê - óáðàòü
				}
				else
				{
					throw;
...
void CFlylinkDBManager::push_json_statistic(const std::string& p_value, const string& p_type, bool p_is_stat_server)
{
	if (!p_value.empty() && BOOLSETTING(USE_FLY_SERVER_STATICTICS_SEND))
	{
		...
		try
		{
			...
			// TODO stat_time ïîêà íå èñïîëüçóåòñÿ, íî ïóñòü áóäåò :)
...
{
	p_is_error = false;
	if (CFlyServerConfig::g_is_use_statistics && BOOLSETTING(USE_FLY_SERVER_STATICTICS_SEND)) // Îòñûëêà ñòàòèñòèêè ðàçðåøåíà?
	{
		try
		{
			...
			if (m_count_json_stat && !ClientManager::isBeforeShutdown())
			{
				...
				// where flush_time is null (ïîêà íå èñïîëüçóåì ýòî ïîëå è íå õðàíèì ëîêàëüíóþ ñòàòèñòèêó - íå ïðèäóìàë êàê ïîä÷èùàòü äàííûå)
			...
			if (l_json_array_id.size() < 50)
				m_count_json_stat = 0; // Âñå çàïèñè áóäóò óäàëåíû íèæå. ñêèäûâàåì ñ÷åò÷èê â íîëü ÷òîáû íå äåëàòü áîëüøå ñåëåêòà
			else
				m_count_json_stat = 1; // Çàïèñè åùå âîçìîæíî åñòü - ÷åðåç ìèíóòó ïðîáóåì ñíîâà.
			if (!l_json_array_id.empty())
			{
				CFlyLock(m_cs);
				// Îòìåòèì ôàêò ïåðåñûëêè ñòàòèñòèêè íà ñåðâåð
		...
		{
			{
				m_select_location_lost.init(m_flySQLiteDB,
				                            "select ip from location_db.fly_location_ip_lost where is_send_fly_server is null limit 100"); // Çà îäèí ðàç áîëüøå 100 íå øëåì
			...
			// Îòìåòèì ôàêò ïåðåñûëêè ìàññèâà IP íà ñåðâåð
	...
	if (is_fly_location_ip_valid())
	{
		if (m_lost_location_cache.find(p_ip) == m_lost_location_cache.end())
		{
			try
			{
				...
				{
					// Ãàñèì ïîïûòêó âñòàâèòü äóáëèêàòíóþ çàïèñü - ýòî íîðìàëüíî.
				}
...
{
	dcassert(p_ip);
	CFlyLock(m_cs); // Áåç ýòîãî ïàäàåò ïî÷åìó-òî
	...
	try
	{
		...
		// TODO - ñêëåèòü âûáîðêó â îäèí çàïðîñ
		// äëÿ ñòðàí è p2p íå çàïðàøèâàòü ïðèâàòíûå àäðåñà
		...
		dcassert(l_count_country <= 1); // Âòîðîãî äèàïàçîíà â GeoIPCountryWhois.csv áûòü íå äîëæíî!
	}
	catch (const database_error& e)
	{
...
{
	CFlyLock(m_cs); // TODO áåç ëîêà ïàäàåì
	int l_result = 0;
	try
	{
		CFlyReadLock(*m_virus_cs);
...
int CFlylinkDBManager::sync_antivirus_db(const string& p_antivirus_db, const uint64_t p_unixtime)
{
	...
	{
		CFlyBusy l_disable_log(g_DisableSQLtrace);
		CFlyLock(m_cs); // TODO - Âåðíóë ëîê - íå ïîíÿòíî ïî÷åìó-òî èíîãäà ïàäàåò
#ifdef _DEBUG
...
{
	CFlyLock(m_cs); // Óáèðàòü íåëüçÿ - ïàäàåì  SQLite - load_registry: library routine called out of sequence
	try
	{
...
void CFlylinkDBManager::delete_torrent_resume(const libtorrent::sha1_hash& p_sha1)
{
	...
	try
	{
		...
		{
			dcassert(0); // Çîâåì âòîðîé ðàç óäàëåíèå - íå õîðîøî
		}
	}
	catch (const database_error& e)
	{
		...
		try
		{
			...
			vector<__int64> l_bad_targets;
			{
				...
				while (l_q.read() && !ClientManager::isBeforeShutdown())
				{
					...
					try
					{
						l_target = QueueManager::checkTarget(l_tgt, l_size, false); // Âàëèäàöèÿ íå ïðîâîäèòü - â áàçå óæå õðàíèòüñÿ õîðîøî
						if (l_target.empty())
						{
							dcassert(0);
					...
					if (l_tempTarget.length() >= MAX_PATH)
					{
						...
						if (l_tempTarget.length() - i >= MAX_PATH || i == string::npos) // Èìÿ ôàéëà áîëüøå MAX_PATH - îáðåæåì
						{
					...
					QueueItemPtr qi = QueueManager::FileQueue::find_target(l_target); //TODO ïîñëå îòêàçà îò êîíâåðòàöèè XML âàðèàíòà î÷åðåäè ìîæíî óäàëèòü
					if (!qi)
					{
...
void CFlylinkDBManager::add_sourceL(const QueueItemPtr& p_QueueItem, const CID& p_cid, const string& p_nick, int p_hub_id)
{
	...
	if (!p_cid.isZero())
	{
		UserPtr l_user = ClientManager::createUser(p_cid, p_nick, p_hub_id); // Ñîçäàåì þçåðà â ëþáîì ñëó÷àå
		bool wantConnection = false;
		try
		{
			CFlyLock(*QueueItem::g_cs);
			//TODO- LOCK ??      QueueManager::LockFileQueueShared l_fileQueue;
			wantConnection = QueueManager::addSourceL(p_QueueItem, l_user, 0, true) && l_user->isOnline(); // Äîáàâèòü ôëàã óñêîðåííîé çàãðóçêè ïåðâûé ðàç.
...
void CFlylinkDBManager::delete_queue_sourcesL(const __int64 p_id)
{
	...
	// TODO - ñäåëàòü àïäåéò ïîëÿ-ôëàæêà + ôèëüòð + ôîíîâàÿ çà÷èñòêà
	...
	try
	{
		...
		if (l_sources.size())
		{
			...
			for (auto j = l_sources.cbegin(); j != l_sources.cend(); ++j)
			{
				...
				l_sql_source->executenonquery(); // TODO - êîïèïàñò
#ifdef _DEBUG
...
bool CFlylinkDBManager::merge_queue_itemL(QueueItemPtr& p_QueueItem)
{
	...
	try
	{
		...
		else
		{
			...
			{
				// Èñòî÷íèêè ïèñàëè â áàçó - åñòü ÷òî óäàëÿòü?
#ifdef _DEBUG
...
bool CFlylinkDBManager::load_last_ip_and_user_stat(uint32_t p_hub_id, const string& p_nick, uint32_t& p_message_count, boost::asio::ip::address_v4& p_last_ip)
{
	...
	try
	{
		...
		if (l_find_cache_item == m_last_ip_cache.end()) // Õàá ïåðâûé ðàç? (TODO - äîáàâèòü çàäåðæêó íà êîë-âî çàïðîñîâ. åñëè áîëüøå N - âûïîëíèòü ïàêåòíóþ çàãðóçêó)
		{
		...
		auto& l_hub_cache = m_last_ip_cache[p_hub_id]; // TODO - óáðàòü ëèøíèé ïîèñê. åãî íàøëè óæå âûøå
...
bool CFlylinkDBManager::load_ratio(uint32_t p_hub_id, const string& p_nick, CFlyUserRatioInfo& p_ratio_info, const boost::asio::ip::address_v4& p_last_ip)
{
	...
	*/
	...
	try
	{
		if (!p_last_ip.is_unspecified()) // Åñëè íåò â òàáëèöå user_db.user_info, â fly_ratio ìîæíî íå õîäèòü - òàì íè÷åãî íåò
		{
			CFlyLock(m_cs); // TODO - óáèðàòü íåëüçÿ ïàäàåì https://drdump.com/Problem.aspx?ProblemID=118720
			m_select_ratio_load.init(m_flySQLiteDB, "select upload,download,(select name from fly_dic where id = dic_ip) " // TODO ïåðåâåñòè íà õðàíåíèå IP êàê ÷èñëà?
			...
			while (l_q.read())
			{
				...
				// dcassert(!l_ip_from_ratio.empty()); // TODO - ñäåëàòü çà÷èñòêó òàêèõ
                                                    ...
                                                    bool& p_is_sql_not_found)
{
	...
	try
	{
		...
		// TODO ïðîâåñòè êîíâåðòàöèþ â äðóãîé ôîðìàò è ôàéë ÁÄ + îòêàçàòüñÿ îò DIC
		...
		if (l_is_exist_map)
		{
			for (auto i = p_user_ratio.getUploadDownloadMap()->begin(); i != p_user_ratio.getUploadDownloadMap()->end(); ++i)
			{
				...
				if (l_last_ip_id &&  // Êîííåêò åùå íå íàñòóïèë - íå ïèøåì â áàçó 0
				        i->second.is_dirty() &&
				        (i->second.get_upload() != 0 || i->second.get_download() != 0)) // Åñëè âñå ïî íóëÿì - òîæå ñòðàííî
				{
		...
		// Èíà÷å ôèêñèðóåì òîëüêî ïîñëåäíèé IP è c÷åò÷èê ìåññàã
...
#ifdef _DEBUG
						{
							// Ïðîâåðèì ÷òî äàííûå íå çàòèðàþòñÿ
							...
							if (l_q.read())
							{
								...
								if (l_message_count > i->second.m_message_count)
								{
									...
									// Â áàçå îêàçàëîñü çíàåíèå áîëüøå ÷åì ïèøåòñÿ - íå çàòèðàåì åãî íóæíî ðàçáèðàòüñÿ êîãäà òàê ïîëó÷àåòñÿ
	...
	else
	{
		if (!p_last_ip.is_unspecified())
		{
			...
			if (p_is_sql_not_found == true || m_update_last_ip.sqlite3_changes() == 0)
			{
				...
				// Ïðîâåðèì íàëè÷èå â áàçå çàïèñè
				{
...
void CFlylinkDBManager::clean_fly_hash_blockL()
{
	...
	// TODO - Âåðñèÿ 2 delete from fly_hash_block where not exists (select 1 from fly_file ff where ff.tth_id = fly_hash_block.tth_id)
}
//========================================================================================================
	...
	{
		{
			...
			*/
			// Çà÷èùàåì ìóñîðîê, êîòîðûé îñòàëñÿ â ôàéëàõ.
			{
...
void CFlylinkDBManager::scan_path(CFlyDirItemArray& p_directories)
{
	...
	if (!p_directories.empty())
	{
		...
		try
		{
			// Òóò òðàíçàêöèþ íåëüÿ - ðàáîòàåì áåç áëîêèðîâêè
			// sqlite3_transaction l_trans(m_flySQLiteDB, p_directories.size() > 1);
			for (auto j = p_directories.begin(); j != p_directories.end(); ++j)
			{
				prepare_scan_folder(Text::toT(j->m_path)); // TODO - õðàíèòü ñðàçó â tstring è îòäàâàòü òóäà ID êàòàëîãà?
			}
			//l_trans.commit();
		}
		catch (const database_error& e)
...
void CFlylinkDBManager::load_path_cache()
{
	...
	/* Îòðóáàåì çàãðóçêó âñåõ êàòàëîãîâ ïà÷êîé - òóïèò íà ãðÿçíîé áàçå
	    CFlyLogFile log(STRING(RELOAD_DIR));
	    // CFlyLock(m_cs); // ïîïðîáóåì åùå ðàç îòêëþ÷èòü ëîê - õîòÿ ðàíüøå òóò ïàäàëè https://drdump.com/Problem.aspx?ProblemID=118720
	    {
	        CFlyFastLock(m_path_cache_cs);
	        m_path_cache.clear();
	    }
...
{
	// Ýòîò çàïðîñ äîëæåí âñåãäà âîçâðàùàòü ïóñòîòó
...
void CFlylinkDBManager::load_dir(__int64 p_path_id, CFlyDirMap& p_dir_map, bool p_is_no_mediainfo)
{
	try
	{
		...
		// TODO - åñëè êàòàëîã íå ñîäåðæèò èíôû - íå çàáèðàòü êîëîíêè èç áàçû
		// Äëÿ ðåàëèçàöèè íóæíî åùå îäíî ïîëå - ôëàã â òàáëèöå fly_path
		// Ïîêà íå ïîíÿòíî êàê åãî àêòóàëèçàöèðîâàòü
		sqlite3_reader l_q = l_sql->executereader();
		...
		while (l_q.read())
		{
			...
			if (!p_is_no_mediainfo) // Ïîäñêàçêà èç êýøà êàòàëîãà.
			{
				const string& l_audio = l_q.getstring(11); // TODO âûòàùèòü äëèòåëüíîñòü â îòäåëüíûé àòðèáóò "4mn 26s | MPEG, 2.0, 128 Kbps"
		...
		{
			// TODO - âûíåñòè â îòåäëüíûé ìåòîä
			CFlyLock(m_cs);
                                  ...
                                  int64_t p_Size, int64_t p_TimeStamp, TTHValue& p_out_tth)
{
	...
	try
	{
		...
		// TODO - òóò èíäåêñ ïî tth_id íå ìåøàåòñÿ?
		// Ïðîòåñòèðîâàòü è çàáðàòü tth èç fly_hash_block ïîäçàïðîñîì
		//
...
bool CFlylinkDBManager::get_tree(const TTHValue& p_root, TigerTree& p_tt, __int64& p_block_size)
{
	...
	try
	{
		...
		CFlyLock(m_cs); // TODO - íèæå èäóò òîëüêî ñåëåêòû - ìîæåò îí íå íóæåí
		...
		if (l_q.read())
		{
			...
			if (l_file_size <= MIN_BLOCK_SIZE) // TODO - òóò âîçìîæíî ýòîãî äåëàòü íåëüçÿ.
			{
				CFlyFastLock(g_tth_cache_cs);
	...
	try
	{
		if (!l_local_map.empty())
		{
			...
			for (auto i = l_local_map.begin(); i != l_local_map.end(); ++i)
			{
				...
				{
					merge_mediainfoL(l_tth_id, i->second.m_path_id, l_name, i->second.m_out_media); // Åñëè ìåäèàèíôó ðàñ÷èòàëè - ñêèíåì åå â áàçó
				}
			}
			l_trans.commit();
		}
...
__int64 CFlylinkDBManager::add_treeL(const TigerTree& p_tt)
{
	{
		CFlyFastLock(g_tth_cache_cs);
		g_tiger_tree_cache.erase(p_tt.getRoot()); // Ñáðîñèì êýø, ÷òîáû ñëó÷àéíî íå äîñòàòü ñòàðóþ êàðòó.
	}
	try
	{
...
CFlylinkDBManager::FileStatus CFlylinkDBManager::get_status_file(const TTHValue& p_tth)
{
	...
	if (m_TTHLevelDB.is_open())
	{
		...
		return static_cast<FileStatus>(l_result); // 1 - ñêà÷èâàë, 2 - áûë â øàðå, 3 - 1+2 è òî è òî, 4- âèðóñíÿ
	}
	else
	{
		CFlyLock(m_cs);
		try
		{
			...
			return static_cast<FileStatus>(l_result); // 1 - ñêà÷èâàë, 2 - áûë â øàðå, 3 - 1+2 è òî è òî :)
		}
		catch (const database_error& e)
		{
...
CFlyLevelDB::~CFlyLevelDB()
{
	...
	// íåëüçÿ óäàëÿòü - ïàäàåì. safe_delete(m_options.env);
