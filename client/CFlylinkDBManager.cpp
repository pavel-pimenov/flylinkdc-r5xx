	...
	try
	{
		...
		*/
		...
		m_flySQLiteDB.executenonquery("CREATE TABLE IF NOT EXISTS antivirus_db.fly_suspect_user("
		                              "nick text not null,ip4 int not null,share int64 not null);"); // ,unixtime int64 not null
...
//========================================================================================================
void CFlylinkDBManager::purge_antivirus_db(uint64_t p_delete_counter)
{
	Lock l(m_cs);
	try
	{
...
//========================================================================================================
int CFlylinkDBManager::sync_antivirus_db(const string& p_antivirus_db, uint64_t p_unixtime)
{
		...
		try
		{
			...
			if (!m_merge_antivirus_db.get())
				m_merge_antivirus_db = auto_ptr<sqlite3_command>(new sqlite3_command(m_flySQLiteDB,
				                                                                     "insert or replace into antivirus_db.fly_suspect_user(nick,share,ip4) values(?,?,?)"));
			int l_pos = 0;
			int l_nick_pos = 0;
			int l_nick_len = 0;
			string l_ip4;
...
//========================================================================================================
int CFlylinkDBManager::calc_antivirus_flag(const string& p_nick, const boost::asio::ip::address_v4& p_ip4, int64_t p_share)
{
	int l_result = 0;
	...
	if (m_virus_user.find(p_nick) != m_virus_user.end())
	{
		...
		m_find_virus_nick = auto_ptr<sqlite3_command>(new sqlite3_command(m_flySQLiteDB,
		                                                                  "select ip4,share from antivirus_db.fly_suspect_user where nick=?"));
...
bool CFlylinkDBManager::load_last_ip_and_user_stat(uint32_t p_hub_id, const string& p_nick, uint32_t& p_message_count, boost::asio::ip::address_v4& p_last_ip)
{
	try
	{
		...
		if (m_virus_user.empty())
		{
			...
			//{
			//  m_antivirus_cache["!"] = CFlyAntivirusItem(); // Пустышка. TODO - поправить немного
			//}
		}
...
void CFlylinkDBManager::sweep_db()
{
	...
	{
		errorDB("SQLite - sweep_db: " + e.getError()); // TODO translate
	}
}
//========================================================================================================
	...
	if (hFind != INVALID_HANDLE_VALUE)
	{
		do
		{
			        ...
			        (l_folder_name != Util::m_dot_dotT))
			{
				...
				if (m_path_cache.find(l_lower_folder_name) == m_path_cache.end())
				{
...
void CFlylinkDBManager::scan_path(CFlyDirItemArray& p_directories)
{
	...
	{
		errorDB("SQLite - scan_path: " + e.getError()); // TODO translate
		return;
	}
}
//========================================================================================================
void CFlylinkDBManager::load_path_cache()
{
	...
	{
		errorDB("SQLite - load_path_cache: " + e.getError()); // TODO translate
		return;
	}
}
//========================================================================================================
...
//========================================================================================================
__int64 CFlylinkDBManager::find_path_idL(const string& p_path)
{
	...
	const __int64 l_last_path_id = m_get_path_id.get()->executeint64_no_throw();
	return l_last_path_id;
}
//========================================================================================================
__int64 CFlylinkDBManager::get_path_idL(string p_path, bool p_create, bool p_case_convet, bool& p_is_no_mediainfo, bool p_sweep_path)
{
	...
	try
	{
		...
		m_last_path_id = find_path_idL(p_path);
	...
	{
		errorDB("SQLite - get_path_idL: " + e.getError()); // TODO translate
	}
	return 0;
}
//========================================================================================================
		...
		if (!p_is_no_mediainfo)
		{
			if (!m_load_dir_sql.get())
				m_load_dir_sql = auto_ptr<sqlite3_command>(new sqlite3_command(m_flySQLiteDB,
				                                                               "select size,stamp,(select tth from fly_hash_block fhb where ff.tth_id= fhb.tth_id) tth,"
				                                                               "name,hit,stamp_share,ftype,bitrate,media_x,media_y,media_video,media_audio from fly_file ff where dic_path=?"
		...
		else
		{
			if (!m_load_dir_sql_without_mediainfo.get())
				m_load_dir_sql_without_mediainfo = auto_ptr<sqlite3_command>(new sqlite3_command(m_flySQLiteDB,
				                                                             "select size,stamp,(select tth from fly_hash_block fhb where ff.tth_id= fhb.tth_id) tth,"
				                                                             "name,hit,stamp_share,ftype from fly_file ff where dic_path=?"
...
void CFlylinkDBManager::flush_hash()
{
	...
	{
		errorDB("SQLite - flush_hash: " + e.getError()); // TODO translate
	}
}
//========================================================================================================
...
bool CFlylinkDBManager::merge_mediainfoL(const __int64 p_tth_id, const __int64 p_path_id, const string& p_file_name, const CFlyMediaInfo& p_media)
{
	...
	{
		errorDB("SQLite - merge_mediainfoL: " + e.getError()); // TODO translate
	}
	return false;
}
//========================================================================================================
                                      ...
                                      bool p_case_convet, __int64& p_path_id)
{
	Lock l(m_cs);
	sqlite3_transaction l_trans(m_flySQLiteDB);
	const auto l_res = merge_fileL(p_Path, p_file_name, p_time_stamp, p_tt, p_case_convet, p_path_id);
	l_trans.commit();
	return l_res;
}
//========================================================================================================
                                       ...
                                       bool p_case_convet, __int64& p_path_id)
{
	...
	{
		errorDB("SQLite - merge_fileL: " + e.getError()); // TODO translate
	}
	return l_tth_id;
}
//========================================================================================================
...
__int64 CFlylinkDBManager::add_treeL(const TigerTree& p_tt)
{
	...
	{
		errorDB("SQLite - add_treeL: " + e.getError()); // TODO translate
	}
	dcassert(0);
	return 0;
}
