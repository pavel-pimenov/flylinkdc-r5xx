...
string ZipFileException::TranslateError(int e)
{
	switch (e)
	{
		...
		default:
			return "unknown error (" + Util::translateError(e) + ")";
	}
}
	...
	for (auto i = files.cbegin(); i != files.cend(); ++i)
	{
		...
		else
		{
			...
			try
			{
				...
				if (f.getSize() != -1)
				{
					time_t time = f.getLastWriteTime();
...
void ZipFile::CreateZipFileList(StringPairList& files, const string& srcPath, const string& dstPath, const string& aPattern, bool keepEmpty) noexcept
{
	;
#ifdef _WIN32
	for (FileFindIter i(srcPath + "*"); i != FileFindIter::end; ++i)
	{
#else
	for (FileFindIter i(srcPath); i != end; ++i)
	{
		...
		if (!aPattern.empty())
		{
			try
			{
				std::regex reg(aPattern);
				if (!std::regex_search(name.begin(), name.end(), reg))
					continue;
			}
			catch (...)
			{
