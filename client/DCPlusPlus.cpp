...
#define LOAD_STEP_L(locale_key, load_function)\
	...
	// «агрузку конфига нужно делать раньше
...
#ifdef USE_FLYLINKDC_VLD
	VLDDisable(); // TODO VLD показывает там лики - не пон€л пока как победить OpenSSL
#endif
// FLYLINKDC_CRYPTO_DISABLE
	LOAD_STEP("SSL", CryptoManager::newInstance());
#ifdef USE_FLYLINKDC_VLD
	VLDEnable(); // TODO VLD показывает там лики - не пон€л пока как победить OpenSSL
#endif
...
#endif
#ifdef _DEBUG
		dcdebug("shutdown start - User::g_user_counts = %d OnlineUser::g_online_user_counts = %d\n", int(User::g_user_counts), int(OnlineUser::g_online_user_counts)); // [!] IRainman fix: Issue 1037 [BUG] иногда тер€ем объект User? https://code.google.com/p/flylinkdc/issues/detail?id=1037
#endif
#ifdef STRONG_USE_DHT
...
#endif
#endif
		preparingCoreToShutdown(); // «овем тут второй раз т.к. веро€тно при автообновлении оно не зоветс€.
...
// FLYLINKDC_CRYPTO_DISABLE
		...
		// [!] IRainman fix: Issue 1037 [BUG] иногда тер€ем объект User? https://code.google.com/p/flylinkdc/issues/detail?id=1037
#ifdef _DEBUG
