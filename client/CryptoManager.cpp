...
#ifdef HEADER_OPENSSLV_H
CriticalSection* CryptoManager::cs = nullptr; // [!] IRainman
#else
...
// #define  FLYLINKDC_TEMP_EXCLUDE_OPENSSL
	...
	if (clientContext && clientVerContext && serverContext && serverVerContext)
	{
		...
		const static unsigned char dh4096_p[] =
		{
...
CryptoManager::~CryptoManager()
{
	...
	// after SSL_library_init calling.
...
void CryptoManager::generateCertificate()
{
	...
	{
		throw CryptoException(STRING(PRIVATE_KEY_FILE_NOT_CHOSEN)); //[!]NightOrion(translate)
	}
	if (SETTING(TLS_CERTIFICATE_FILE).empty())
	{
		throw CryptoException(STRING(CERTIFICATE_FILE_NOT_CHOSEN)); //[!]NightOrion(translate)
	}
...
#define CHECK(n) if(!(n)) { throw CryptoException(#n); }
	...
	CHECK((X509_NAME_add_entry_by_txt(nm, "CN", MBSTRING_ASC,
	                                  (const unsigned char*)ClientManager::getMyCID().toBase32().c_str(), -1, -1, 0)))
...
void CryptoManager::loadCertificates() noexcept
{
	...
	*/
	...
	for (auto i = certs.cbegin(); i != certs.cend(); ++i)
	{
		if (
		...
		{
			LogManager::message(STRING(FAILED_TO_LOAD_TRUSTED_CERTIFICATE_FROM)  + ' ' +  *i);
		}
	}
	loadKeyprint();
	certsLoaded = true;
...
bool CryptoManager::checkCertificate() noexcept
{
	...
	if (cn != ClientManager::getMyCID().toBase32())
	{
		return false;
	}
...
FILE* CryptoManager::openSertFile() noexcept
{
	...
	if (!f)
	{
		LogManager::message("Unable open cert file: " + SETTING(TLS_CERTIFICATE_FILE));
	}
	return f;
}
...
void CryptoManager::decodeBZ2(const uint8_t* is, size_t sz, string& os)
{
	...
	bs.avail_in = static_cast<unsigned int>(sz);
	bs.avail_out = static_cast<unsigned int>(bufsize);
