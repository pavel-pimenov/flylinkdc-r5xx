...
const FavoriteHubEntry* Client::reloadSettings(bool updateNick)
{
	...
	if (hub)
	{
		if (updateNick)
		{
			...
			if (!getRandomTempNick().empty()) // ñãåíåðèëè _Rxxx?
...
void Client::send(const char* aMessage, size_t aLen)
{
	if (!isReady())
	{
		...
		//dcassert(isReady()); // Ïîä îòëàäêîé ïàäàåì òóò. íàéòè ïðè÷èíó.
		return;
	}
	updateActivity();
	{
...
uint64_t Client::search_internal(const SearchParamToken& p_search_param)
{
	...
	// TODO - ðàçîáðàòüñÿ ñ ýòèì ìåñòîì.
	// ìåðòâûé êîä
	...
	l_search_param_token.m_owner = p_search_param.m_owner; // Ðàíüøå òóò åãî íå áûëî.
...
void Client::on(Second, uint64_t aTick) noexcept
{
	...
	if (isConnected() && !m_searchQueue.empty())
	{
		...
		{
			// TODO - ïðîáåæàòüñÿ ïî áèòîâîé ìàñêå?
			// Åñëè îíà òàì åñòü
			SearchParamToken l_search_param_token;
	...
	if (is_all_my_info_loaded() && CFlyServerConfig::g_interval_flood_command)
	{
		if (!CFlyServerConfig::isIgnoreFloodCommand(p_command))
		{
			...
			if (l_flood_find.second == false)
			{
				...
				{
					// Ïðîøëà ñåêóíäà è êîìàíä ïðèøëî áîëüøå 100 (CFlyServerConfig::g_max_flood_command)
					// ëîãèðóåì ñ÷åò÷èê è áàíèì íà 20 ñåêóíä äàííûå êîìàíäû (CFlyServerConfig::g_ban_flood_command)
					if (l_result.m_count > CFlyServerConfig::g_max_flood_command)  // â ñåêóíäó áîëüøå ÷åì 20
					{
						if (l_result.m_is_ban == false) // Â ëîã êèäàåì ïåðâóþ ìåññàãó
						{
							if (BOOLSETTING(LOG_FLOOD_TRACE))
							{
						...
						if (l_delta > CFlyServerConfig::g_ban_flood_command * 1000) // 20 ñåêóíä äàííûå êîìàíäû â áàíå!
						{
							if (BOOLSETTING(LOG_FLOOD_TRACE))
							{
					...
					{
						// Êîìàíä ïðèáåæàëî ìàëî - çà÷èùàåìñÿ
						l_result.m_is_ban = false;
						l_result.m_count = 0;
						l_result.m_start_tick = l_result.m_tick = GET_TICK();
						if (BOOLSETTING(LOG_FLOOD_TRACE))
...
#endif
		if (QueueManager::handlePartialSearch(aTTH, partialInfo)) // TODO - ÷àñòî èùåòñÿ ïî ÒÒÕ
		{
#ifdef _DEBUG
			...
			try
			{
				...
				Socket l_udp;
				l_udp.writeTo(Socket::resolve(l_ip), l_port, cmd.toString(ClientManager::getMyCID())); // TODO - çà÷åì òóò resolve êðîìå IP ìîæåò áûòü ÷òî-òî äðóãîå?
