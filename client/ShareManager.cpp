	...
	if (aXml.findChild("LostShare"))
	{
		...
		while (aXml.findChild("Directory"))
		{
			...
			if (g_shares.find(realPath) == g_shares.end())
			{
				...
				const string vName = validateVirtual(virtualName.empty() ? Util::getLastDir(realPath) : virtualName);
				g_lost_shares.insert(std::make_pair(realPath, CFlyBaseDirItem(vName, 0)));
				if (File::isExist(realPath)) // Åñëè êàòàëîã ïîÿâèëñÿ - äîáàâèì åãî â çàïðîñ íà âîçâðàò â øàðó...
				{
...
struct ShareLoader : public SimpleXMLReader::CallBack
{
		...
		void startTag(const string& p_name, StringPairList& p_attribs, bool p_simple)
		{
			...
			else if (cur && p_name == g_SFile)
			{
				...
				f->initLowerName();
				if (it.second && p_attribs.size() > 4) // Ýòî óæå íàøà øàðà. òóò ìåäèàèíôà åñëè áîëüøå 4-õ.
				{
					...
					{
						dcassert(!(!l_audio.empty() || !l_video.empty())); // Ýòîãî íå äîëæíî áûòü?
					}
				}
			}
		}
...
bool ShareManager::loadCache() noexcept
{
	try
	{
		...
		l_cache_loader_log.step("parse xml done");
		{
			...
			if (getShareSize() >= 0)
			{
				// Ïîëó÷èëè ðàçìåð øàðû èç êýøà - íå âûïîëíÿåì ïîâòîðíûé îáõîä â internalCalcShareSize();
...
void ShareManager::addDirectory(const string& realPath, const string& virtualName, bool p_is_job)
{
	...
	if (!checkVirtual(l_realPathWoPS))  // [5] https://www.box.net/shared/a877c0d7ef9f93b93c55  TODO - Çàïðåòèòü âûáðîñ èñêëþ÷åíèé èç ìàñòåðà èëè ãàñèòü èõ â íåì.
	{
...
void ShareManager::removeDirectory(const string& realPath)
{
	...
	
	{
		...
		const string l_Name = i->second.m_synonym; // Ññûëêó íå äåëàòü. fix http://www.flickr.com/photos/96019675@N02/9515345001/
		{
	...
	for (FileFindIter i(aName + '*'); !ClientManager::isBeforeShutdown() && i != FileFindIter::end; ++i)
	{
		...
		else
		{
			...
			if (!g_fly_server_config.isBlockShareExt(l_lower_name, l_ext))
			{
				...
				if (stricmp(l_PathAndFileName, SETTING(TLS_PRIVATE_KEY_FILE)) == 0) // TODO - óíåñòè ïðîâåðêó â äðóãîå ìåñòî.
					continue;
...
int ShareManager::run()
{
	...
	if (g_is_first == false)
	{
		...
		for (int i = 0; i < 50 * 10; i++) // Æäåì 5 ñåê
		{
			::Sleep(10);
			if (ClientManager::isBeforeShutdown())
			{
...
void ShareManager::Directory::toXmlL(OutputStream& xmlFile, string& p_indent, string& tmp2, bool fullList) const
{
	...
	xmlFile.write(SimpleXML::escapeAtrib(getName(), tmp2)); // TODO  óïðîñòèë îïåðàöèþ escape áåç àíàëèçà êîäèðîâêè utf-8 stricmp(encoding, Text::g_utf8)
...
void ShareManager::Directory::filesToXmlL(OutputStream& xmlFile, string& indent, string& tmp2) const
{
	for (auto i = m_share_files.cbegin(); i != m_share_files.cend(); ++i)
	{
		...
		xmlFile.write(SimpleXML::escapeAtrib(f.getName(), tmp2)); // TODO - óïðîñòèë îïåðàöèþ escape áåç àíàëèçà êîäèðîâêè utf-8 stricmp(encoding, Text::g_utf8)
...
#ifdef _DEBUG
// Âñå ðàñøèðåíèÿ 1-ãî òèïà äîëæíû áûòü ñ òî÷êîé è äëèíîé 4
...
// Âñå ðàñøèðåíèÿ 2-ãî òèïà äîëæíû áûòü òîæå ñ òî÷êîé íî ñ äëèíîé îòëè÷íîé îò 4
...
//		LogManager::message(l_buf);
			...
			{
#ifdef _DEBUG
//			LogManager::message("[ìèíèìóì] aSizeMode == Search::SIZE_ATLEAST && aSize > i->getSize()");
#endif
				continue;
			}
			...
			{
#ifdef _DEBUG
//			LogManager::message("[ìàêñèìóì] aSizeMode == Search::SIZE_ATMOST && aSize < i->getSize()");
#endif
				continue;
			}
			auto j = cur->begin();
...
bool ShareManager::search_tth(const TTHValue& p_tth, SearchResultList& aResults, bool p_is_check_parent)
{
	...
	if (!g_RebuildIndexes) // https://drdump.com/DumpGroup.aspx?DumpGroupID=382746&Login=guest
	{
		...
		// TODO - äëÿ TTH ñèëüíî òîëñòûé îáúåêò  SearchResult
...
void ShareManager::search(SearchResultList& aResults, const SearchParam& p_search_param) noexcept
{
	...
	{
		return; // Óõîäèì ñðàçó - ó íàñ â øàðå ýòîãî íå ïîÿâèëîñü.
	}
	...
	const StringList& sl = t.getTokens();
	{
		...
		if (!l_is_bloom)
		{
			addUnknownFile(l_raw_query); // TODO - ìîæåò âûíåñòè bloom â ãëîáàëüíóþ ÷àñòü.
			return;
		}
	}
	
	StringSearch::List ssl; // TODO - êàíäèäàò çàñóíóòü â ñòðóêòóðó
	ssl.reserve(sl.size());
#ifdef FLYLINKDC_USE_COLLECT_STAT
	int l_count_find = 0;
#endif
	...
	// Íè÷åãî íå íàøëè - ñîõðàíèì óñëîâèå ïîèñêà ÷òîáû íå èñêàòü âòîðîé ðàç ïî ýòîìó-æå çàïðîñó.
	if (aResults.empty())
	{
                      ...
                      int64_t aTimeStamp, const CFlyMediaInfo& p_out_media, int64_t p_size) noexcept
{
	dcassert(!ClientManager::isBeforeShutdown());
	{
		...
		{
			if (Directory::Ptr d = getDirectoryL(fname)) // TODO ïðîêèíóòü p_path_id è èñêàòü ïî íåìó?
			{
