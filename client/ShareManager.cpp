	...
	try
	{
		...
		l_cache_loader_log.step("parse xml done");
		{
			...
			if (getSharedSize() >= 0)
			{
				...
				g_CurrentShareSize = getSharedSize(); // TODO зачем нам хранить два значени€ размера шары
			}
			else
			{
...
{
	webrtc::ReadLockScoped l(*g_csTTHIndex); // “ут пока лок по записи т.к. не вынесен.
...
{
	webrtc::ReadLockScoped l(*g_csShareCache);
...
{
	webrtc::WriteLockScoped l(*g_csShareCache);
