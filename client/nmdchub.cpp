...
{
	const StringTokenizer<string> tok(tag, ','); // TODO - убрать разбор токенов. сделать простое сканирование в цикле в поиске зап€тых
	string::size_type j;
	id.setLimit(0);
...
//	LogManager::getInstance()->message("###################################### cmd = " + cmd + " param = " + param);
	...
	if (cmd == "Search")
	{
		...
		if (!isPassive)
		{
			...
			if (i != string::npos)
			{
				const auto k = param.find("?9?TTH:", i); // ≈сли идет запрос по TTH - пропускаем без проверки
				if (k == string::npos)
				{
...
void NmdcHub::myInfoParse(const string& param) noexcept
{
	...
	OnlineUserPtr ou = getUser(nick, false, m_bLastMyInfoCommand == DIDNT_GET_YET_FIRST_MYINFO); // ѕри первом коннекте исключаем поиск
...
void NmdcHub::on(BufferedSocketListener::MyInfoArray, const StringList& p_myInfoArray) noexcept
{
	...
	{
		myInfoParse(toUtf8(*i)); // –азобратьс€ почему тут toUtf8
