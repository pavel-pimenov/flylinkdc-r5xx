...
OnlineUserPtr NmdcHub::getUser(const string& aNick, bool p_hub, bool p_first_load)
{
	...
	OnlineUserPtr ou;
	//if (!p_first_load) ïîêà âîçíèêàþò äóáëè - ïî÷åìó íå ïîíÿòíî
...
{
	const StringTokenizer<string> st(param, ' '); // TODO óáðàòü òåêåíû. ñäåëàòü ïîèñêîì. http://code.google.com/p/flylinkdc/issues/detail?id=1112
...
void NmdcHub::userIPParse(const string& p_ip_list)
{
	if (!p_ip_list.empty())
	{
		...
		fire_user_updated(v); // TODO - ñëàòü ñîîáùåíèÿ î ñìåíå òîëüêî IP
		/*
...
void NmdcHub::myInfo(bool p_always_send, bool p_is_force_passive)
{
	...
	if (p_always_send || l_is_change_my_info || l_is_change_fly_info)
	{
		...
		if ((m_supportFlags & SUPPORTS_EXTJSON) && l_is_change_fly_info)
		{
			...
			boost::replace_all(l_json_str, "\r", " "); // TODO óáðàòü âíóòðü jsoncpp
	...
	try
	{
		...
		else
		{
			...
			if (!ClientManager::isShutdown())
			{
				if (p_is_disable_fire == false)
				{
					fly_fire1(ClientListener::UserUpdated(), ou); // TODO îáíîâëÿòü òîëüêî JSON
				}
			}
		}
		return true;
	...
	if (!tmpDesc.empty() && tmpDesc[tmpDesc.size() - 1] == '>')
	{
		...
		if (x != string::npos)
		{
			...
			if (tmpDesc.length()  > x + 2 && l_is_only_desc_change == false)
			{
				...
				updateFromTag(ou->getIdentity(), l_tag); // òÿæåëàÿ îïåðàöèÿ ñ òîêåíàìè. TODO - îïòèìèçíóòü
			}
