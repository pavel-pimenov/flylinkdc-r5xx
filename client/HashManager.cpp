...
bool HashManager::StreamStore::loadTree(const string& p_filePath, TigerTree& p_Tree, int64_t p_FileSize)
{
	...
	{
		// ќтключил спам http://code.google.com/p/flylinkdc/issues/detail?id=1415
...
{
	dcassert(p_path_id);
	p_out_media.init(); // TODO - делаетс€ двойной инит
...
bool HashManager::Hasher::fastHash(const string& fname, uint8_t* buf, TigerTree& tth, int64_t size)
{
	...
	// TODO - размер сектора определ€ть один раз дл€ одной буквы через массив от A до Z и не звать GetDiskFreeSpaceA
	// на каждый файл
	// TODO - узнать зачем его вообще определ€ть?
						...
						if (m_running)
						{
							...
							do
							{
								...
								if (n > 0) // [+]PPA
								{
									...
									{
										FastLock l(cs);
										m_currentSize = max(static_cast<uint64_t>(m_currentSize - n), static_cast<uint64_t>(0)); // TODO - max от 0 дл€ беззнакового?
									}
									l_sizeLeft -= n;
