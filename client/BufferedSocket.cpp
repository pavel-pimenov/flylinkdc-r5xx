...
void BufferedSocket::resizeInBuf()
{
	...
	do
	{
		...
		{
			l_size /= 2; // «аказываем в 2 раза меньше
                                       ...
                                       CFlySearchArrayFile& p_file_search)
{
	if (p_line.compare(0, 8, "$Search ", 8) == 0)
	{
		if (m_is_hide_share == false)
		{
			...
			// TODO научитьс€ обрабатывать лимит по размеру вида
			// "x.x.x.x:yyy T?F?57671680?9?TTH:A3VSWSWKCVC4N6EP2GX47OEMGT5ZL52BOS2LAHA"
			if (l_marker_tth != string::npos && l_marker_tth > 5 && l_line_item[l_marker_tth - 4] == ' ') // ѕоправка на полную команду  F?T?0?9?TTH: или F?F?0?9?TTH: или T?T?0?9?TTH:
			{
				l_marker_tth -= 4;
...
void BufferedSocket::all_myinfo_parser(const string::size_type p_pos_next_separator, const string& p_line, StringList& p_all_myInfo, bool p_is_zon)
{
	...
	if (m_is_all_my_info_loaded == false)
	{
		...
		else if (m_myInfoCount)
		{
			...
			m_is_all_my_info_loaded = true; // закончилс€ стартовый поток $MyINFO
		}
	}
	if (p_all_myInfo.empty())
	{
		...
		{
			fire(BufferedSocketListener::Line(), l_line_item); // TODO - отказатьс€ от временной переменной l и скользить по окну inbuf
		}
	}
}
...
/*
                                    ...
                                    if (!g_is_test)
                                    {
                                        ...
                                        {
                                            char bbb[200];
                                            snprintf(bbb, sizeof(bbb), "$ALL Guest%d <<Peers V:(r622),M:P,H:1/0/0,S:15,C: емерово>$ $%c$$3171624055$", i, 5);
                                            l_all_myInfo.push_back(bbb);
                                        }
                                    }
#endif // FLYLINKDC_EMULATOR_4000_USERS
#ifdef FLYLINKDC_EMULATOR_4000_USERS
// √енерируем случайные IP-адреса
                                    for (int i = 0; i < l_count_guest; ++i)
                                    {
                                        char bbb[200];
...
//
			...
			case MODE_LINE:
			{
				...
				// ≈сли пасив - отвечаем в буфер сразу
				// ≈сли актив - кидаем отсылку UDP (тоже через очередь?)
