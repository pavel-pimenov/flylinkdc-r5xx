...
void BufferedSocket::resizeInBuf()
{
	...
	do
	{
		...
		catch (std::bad_alloc&)
		{
			...
			l_size /= 2; // Çàêàçûâàåì â 2 ðàçà ìåíüøå
...
//             : new Socket(/*Socket::TYPE_TCP*/));
	...
	s->create(); // â AirDC++ íåò òàêîé õåðíè... ðàçîáðàòüñÿ
                                       ...
                                       CFlySearchArrayFile& p_file_search)
{
	...
	if (p_line.compare(2, 6, "earch ", 6) == 0)
	{
		...
		// TODO íàó÷èòüñÿ îáðàáàòûâàòü ëèìèò ïî ðàçìåðó âèäà
		...
		if (l_marker_tth != string::npos &&
		   ...
		   ) // Ïîïðàâêà íà ïîëíóþ êîìàíäó  F?T?0?9?TTH: èëè F?F?0?9?TTH: èëè T?T?0?9?TTH:
		{
...
void BufferedSocket::all_myinfo_parser(const string::size_type p_pos_next_separator, const string& p_line, StringList& p_all_myInfo, bool p_is_zon)
{
	...
	if (m_is_all_my_info_loaded == false)
	{
		...
		else if (m_myInfoCount)
		{
			...
			set_all_my_info_loaded(); // çàêîí÷èëñÿ ñòàðòîâûé ïîòîê $MyINFO
		}
	}
	if (p_all_myInfo.empty())
	{
		...
		else
		{
			...
			if (!ClientManager::isBeforeShutdown())
			{
				...
				if (m_is_disconnecting == false)
				{
					fly_fire1(BufferedSocketListener::Line(), l_line_item); // TODO - îòêàçàòüñÿ îò âðåìåííîé ïåðåìåííîé l è ñêîëüçèòü ïî îêíó inbuf
				}
			}
		}
	}
...
/*
                                    ...
                                    if (!g_is_test)
                                    {
                                        ...
                                        {
                                            char bbb[200];
                                            snprintf(bbb, sizeof(bbb), "$ALL Guest%d <<Peers V:(r622),M:P,H:1/0/0,S:15,C:Êåìåðîâî>$ $%c$$3171624055$", i, 5);
                                            l_all_myInfo.push_back(bbb);
                                        }
                                    }
#endif // FLYLINKDC_EMULATOR_4000_USERS
#ifdef FLYLINKDC_EMULATOR_4000_USERS
// Ãåíåðèðóåì ñëó÷àéíûå IP-àäðåñà
                                    for (int i = 0; i < l_count_guest; ++i)
                                    {
                                        char bbb[200];
...
#define FLYLINKDC_USE_MYINFO_ARRAY
					...
					while ((pos = l.find(m_separator)) != string::npos)
					{
						...
						l.erase(0, pos + 1 /* separator char */); // TODO íå ýôôåêòèâíî
					}
#endif
					}
					else
				...
				case MODE_LINE:
				{
					...
					//======================================================================
					// TODO - âñòàâèòü áûñòðóþ îáðàáîòêó ïîèñêà ïî TTH áåç âûçîâà ëèñòåíåðîâ
					// Åñëè ïàñèâ - îòâå÷àåì â áóôåð ñðàçó
					// Åñëè àêòèâ - êèäàåì îòñûëêó UDP (òîæå ÷åðåç î÷åðåäü?)
					//======================================================================
...
boost::asio::ip::address_v4 BufferedSocket::getIp4() const
{
	...
	{
		boost::system::error_code ec;
		const auto l_ip = boost::asio::ip::address_v4::from_string(sock->getIp(), ec); // TODO - êîíâåðòíóòü IP è â ñîêåòàõ
		dcassert(!ec);
		return l_ip;
	}
	else
...
void BufferedSocket::threadSendFile(InputStream* p_file)
{
	...
	const size_t l_sockSize = MAX_SOCKET_BUFFER_SIZE; // òîðìîçèò îòäà÷à size_t(sock->getSocketOptInt(SO_SNDBUF));
	...
	ByteVector l_readBuf; // TODO çàìåíèòü íà - íå ïèøåò áóôåðà 0-ÿìè std::unique_ptr<uint8_t[]> buf(new uint8_t[BUFSIZE]);
	...
	while (!socketIsDisconnecting())
	{
		if (!readDone && l_readBuf.size() > readPos)
		{
			...
			size_t actual = p_file->read(&l_readBuf[readPos], bytesRead); // TODO ìîæíî óçíàòü ÷òî ñ÷èòàëè ïîñëåäíèé êóñîê â ôàéë
#ifdef _DEBUG
			if (actual)
			{
				std::ofstream l_fs;
		...
		l_readBuf.resize(g_bufSize);
		l_writeBuf.resize(readPos); // TODO - l_writeBuf îïóñòèòü íèæå è çàêàçûâàòü ñêîëüêî íóæíî.
		readPos = 0;
...
void BufferedSocket::fail(const string& aError)
{
	...
	if (m_state == RUNNING)
	{
		...
		// Íåëüçÿ - âåøàåìñÿ if (!ClientManager::isBeforeShutdown())
		{
