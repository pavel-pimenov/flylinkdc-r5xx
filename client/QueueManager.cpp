...
bool QueueManager::UserQueue::userIsDownloadedFiles(const UserPtr& aUser, QueueItemList& p_status_update_array)
{
	...
	for (size_t i = 0; i < QueueItem::LAST && !ClientManager::isBeforeShutdown(); ++i)
	{
		...
		{
			p_status_update_array.insert(p_status_update_array.end(), j->second.cbegin(), j->second.cend()); // Áåç ëîêà?
...
// Ïðè ïåðâîé çàãðóçêè î÷åðåäè èç áàçû íå çîâåì calcAverageSpeedAndCalcAndGetDownloadedBytesL
			...
			for (auto j = i->second.cbegin(); j != i->second.cend(); ++j)
			{
				...
				if (qi->isWaiting())
				{
					...
					if (l_file_slot == 0 ||
					        ...
					        QueueManager::getRunningFileCount(l_file_slot) < l_file_slot) // TODO - äâîéíîé ëîê
					{
						return qi;
					}
					else
	...
	for (auto i = g_queue.cbegin(); i != g_queue.cend(); ++i)
	{
		...
		if (q->isRunning()) //
		{
			...
			if (l_cnt > p_stop_key && p_stop_key != 0)
				break; // Âûõîäèì ðàíüøå è íå áåãàåì ïî âñåé î÷åðåäè.
		}
		// TODO - ìîæíî íå áåãàòü ïî î÷åðåäè à ñ÷èòàòü â îíëàéíå.
		// Àëãîðèòì:
		// 1. ïðè óäàëåíèè èç g_queue -1 (åñëè m_queue.download çàïîëíåíà)
		// 2  ïðè äîáàâëåíèè ê g_queue +1 (åñëè m_queue.download çàïîëíåíà)
		// 2. ïðè äîáàâëåíèè â g_queue.download +1
		// 3. ïðè îïóñòîøåíèè g_queue.download -1
	}
	return l_cnt;
}
...
void QueueManager::FileQueue::calcPriorityAndGetRunningFilesL(bool p_is_calc_prior, QueueItem::PriorityArray& p_changedPriority, QueueItemList& p_runningFiles)
{
	for (auto i = g_queue.cbegin(); i != g_queue.cend(); ++i)
	{
		...
		if (q->isRunning())
		{
			...
			if (p_is_calc_prior)
			{
				// TODO calcAverageSpeedAndDownloadedBytes - òÿæåëàÿ îïåðàöèÿ çà÷åì çîâåì òàê ÷àñòî?
...
void QueueManager::UserQueue::setQIPriority(const QueueItemPtr& qi, QueueItem::Priority p)
{
	...
	removeQueueItemL(qi);
	qi->setPriority(p); // TODO äëÿ óñòàíîâêè ïðèîðèòåòà - íóæíî óäàëèòü è ñíîâà äîáàâèòü çàïèñü â êîíòåéíåð? - ýòî çîâåòñÿ î÷åíü ÷àñòî
	addL(qi);
}
...
void QueueManager::UserQueue::removeUserL(const QueueItemPtr& qi, const UserPtr& aUser)
{
	...
	}
	{
		...
		// TODO - ïåðåâåñòè íà set const auto& i = uq.find(qi);
		if (i == uq.cend())
		{
...
void QueueManager::on(TimerManagerListener::Minute, uint64_t aTick) noexcept
{
	...
	if (g_fileQueue.getSize() > 0 && aTick >= nextSearch && BOOLSETTING(AUTO_SEARCH))
	{
		...
		QueueItemPtr qi;
		while ((qi = g_fileQueue.findAutoSearch(m_recent)) == nullptr && !m_recent.empty()) // Ìåñòàìè íå ïåðåñòàâëÿòü findAutoSearch ìåíÿåò recent
		{
			m_recent.pop_front();
		}
		if (qi)
                       ...
                       Flags::MaskType aFlags /* = 0 */, bool addBad /* = true */, bool p_first_file /*= true*/)
{
	...
	
	{
		...
		// Ïî TTH èñêàòü íåëüçÿ
		// Ïðîáëåìà îïèñàíà òóò htt
		if (!q)
		{
			if (l_newItem)
			{
...
DownloadPtr QueueManager::getDownload(UserConnection* aSource, string& aMessage) noexcept
{
	...
	// Íåëüçÿ çâàòü new Download ïîä ëîêîì QueueItem::g_cs
...
#endif
	// TODO - ïðèíóäèòåëüíî çàêðûâàòü ôàéë ïî èìåíè â ïóëå
...
void QueueManager::fire_sources_updated(const QueueItemPtr& qi)
{
	if (!ClientManager::isBeforeShutdown())
	{
		...
		// TODO - ïîñ÷èòàòü ñêîëüêî ïîçèöèé ýêîíîìèì
	}
}
			...
			if (q)
			{
				...
				else
				{
					if (aDownload->getType() != Transfer::TYPE_TREE)
					{
						...
						// Íå çàòèðàåì ïóòü ê âðåìåííîìó ôàéëó
						//if (isEmpty)
						//{
...
bool QueueManager::removeTarget(const string& aTarget, bool p_is_batch_remove)
{
	UserList x;
	{
		...
		if (q->isSet(QueueItem::FLAG_DIRECTORY_DOWNLOAD))
		{
			...
			{
				CFlyFastLock(csDirectories);
				for_each(m_directories.equal_range(q->getSourcesL().begin()->first) | map_values, DeleteFunction()); // Ìóòíîå ìåñòî
	...
	string removeRunning;
	{
		...
		while ((qi = g_userQueue.getNextL(aUser, QueueItem::PAUSED)) != nullptr)
		{
			if (qi->isSet(QueueItem::FLAG_USER_LIST))
			{
				...
				if (l_is_found == false)
				{
					break;
					/*
					×èòàåì íàïðèìåð ñ ñ þçåðà ñïèñîê ôàéëîâ. Â òðàíñôåðå (âíèçó, îêíî ïåðåäà÷) âèñèò -1Á ( ó ìåíÿ ãàâíîèíòîðåíòû, íî ìîæíî ïîâòîðèòü íà ÁÎËÜØÎÌ ôàéëëèñòå íàâåðíîå),
					Èä¸ì â Î÷åðåäü Ñêà÷èâàíèÿ. Ñëåâà â äåðåâå - â ÏÊÌ íà File Lists âûáèðàåì Óäàëèòü âñ¸ / Óäàëèòü... áëà-áëà-áëà
					Âíèçó â òðàíñôåðå íà ñòðîêå ñ -1Á äàâèì ÏÊÌ, âûáèðàåì Óäàëèòü ïîëüçîâàòåëÿ èç î÷åðåäè
					Ôëàé âèñíåò íàìåðòâî.
					*/
				}
			}
			else
		...
		if (qi)
		{
			...
			else
			{
				...
				isRunning = true;
				qi->removeSourceL(aUser, reason);
				fire_status_updated(qi); // TODO âîçìîæíî çàëèïàíèå òóò
				fire_sources_updated(qi); // TODO âîçìîæíî çàëèïàíèå òóò
				setDirty();
			}
		}
	}
	...
	if (!ClientManager::isBeforeShutdown())
	{
		...
		if (q != nullptr && q->getPriority() != p)
		{
			...
			if (!isFinished)
			{
				...
				// òóò âîçíèêàåò ïðîáëåìà https://github.com/pavel-pimenov/flylinkdc-r5xx/issues/1692
				//g_userQueue.setQIPriority(q, p); // !!!!!!!!!!!!!!!!!! Óäàëÿåò è âñòàâëÿåò â ìàññèâ êàæäóþ ñåêóíäó
				// Ïîìåíÿë âûçîâ íà
				q->setPriority(p);
				// Îøèáêà àêòèâèðîâàëàñü ìåæäó áèëäàìè 21194 è 21203
				// â ýòîé ðåâèçèè ÿ ÷èíèë áàã - Óáðàë ïðåðûâàíèå çàêà÷êè áîëüøîãî êîë-âî ôàéëîâ (revert r20612)
				// Â îáùåì ïóòíîå ìåñòî.
...
void QueueManager::saveQueue(bool p_force /* = false*/) noexcept
{
				...
				for (auto i = g_fileQueue.getQueueL().begin(); i != g_fileQueue.getQueueL().end(); ++i)
				{
					...
					if (!qi->isAnySet(QueueItem::FLAG_USER_LIST | QueueItem::FLAG_USER_GET_IP))
					{
						        ...
						        qi->isDirtySource() == false)
						{
							...
							qi->setDirtySegment(false); // Ñ÷èòàåì ÷òî îáíîâëåíèå ñåãìåíòîâ ïðîéäåò áåç îøèáîê.
						}
	...
	// Åñëè èçìåíèëèñü òîëüêî ñåãìåíòû + ïðèîðèòåòû - ìîæíî îáíîâèòü áàçó áåç áëîêèðîâêè ìåíåäæåðà î÷åðåäåé
		...
		if (!l_matches.empty())
		{
			...
			for (auto i = l_matches.cbegin(); i != l_matches.cend(); ++i)
			{
				...
				if (qi->getSize() == p_sr->getSize())
				{
					...
					{
						// Íàøëè èñòî÷íèê íî îí íå àêòèâíûé åùå
...
void QueueManager::on(TimerManagerListener::Second, uint64_t aTick) noexcept
{
	...
	QueueItemList l_runningItems;
	{
		...
		if ((g_filter++ % 10) == 0) // Äåëàåì ðàñ÷åò ïðèîðèòåòîâ ðåæå
		{
			RLock(*FileQueue::g_csFQ);
	...
	{
		fly_fire1(QueueManagerListener::Tick(), l_runningItems); // Íåëüçÿ çâàòü ïîä ëîêîì
	}
	{
...
bool QueueManager::handlePartialResult(const UserPtr& aUser, const TTHValue& tth, const QueueItem::PartialSource& partialSource, PartsInfo& outPartialInfo)
{
	...
	
	{
		...
		WLock(*QueueItem::g_cs); // TODO - îïóñòèòü íèæå?
		...
		if (si == qi->getSourcesL().end())
		{
			...
			else
			{
				...
				si = qi->findSourceL(aUser); // TODO - ïîâòîðíûé ïîèñê?
...
void QueueManager::FileQueue::findPFSSourcesL(PFSSourceList& sl)
{
	...
	// TODO - óòàùèòü â îòäåëüíûé ìåòîä
	RLock(*g_csFQ);
	for (auto i = g_queue.cbegin(); i != g_queue.cend(); ++i)
	{
		if (ClientManager::isBeforeShutdown())
		...
		if (q->m_is_file_not_exist == false && !File::isExist(q->isFinished() ? q->getTarget() : q->getTempTargetConst())) // Îáÿçàòåëüíî Const
		{
...
//////////////////
#ifdef _DEBUG
		// Ïîñëå îòëàäêè óáðàòü ñàðûé âàðèàíò íàïîëíåíèÿ
