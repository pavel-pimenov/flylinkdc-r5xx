...
class GPGPUPlatform
{
	public:
		...
		virtual bool krn_ttr(uint8_t *data, uint64_t bc, uint64_t last_bs, uint64_t res[3]) { return false; }
};
...
class GPGPUOpenCL : public GPGPUPlatform
{
	public:
		...
		int get_dev_cnt() { return (int)devs.size(); }
		const string& get_platform_name() { return pfm_name; }
		bool finish();
