...
struct CASLock
{
	void Acquire()
	{
		while (!CAS(mLock, u32(0), u32(1))) YIELD()
		}
	void Release()
	{
		while (!CAS(mLock, u32(1), u32(0))) YIELD()
		}
	bool TryAcquire()
	{
		return CAS(mLock, u32(0), u32(1));
	}
	bool TryRelease()
	{
		return CAS(mLock, u32(1), u32(0));
	}
	u32 Value() const
	{
		return mLock;
...
struct Buffer
{
		...
		Buffer(u32 size) : mBuffer(NULL), mAlloc(0), mItems(0)
		{
			Resize(size);
		}
		~Buffer()
		...
		template< class Mapto > void ForEach(Mapto mapto, u32 limit)
		{
...
struct ColorRamp
{
	...
	ColorRamp &push(const ColorF &color, f32 value)
	{
		mColors.Push(Marker(color, value));
		return *this;
	}
...
struct Caller
{
		...
		*/
		...
		static struct Max
		{
				...
				{
					return mColors.value(f32(f64(u) / f64(u64fields[e])));
				}
				const char *color(f64Enum e, f64 f) const
				{
					return mColors.value(f32(f / f64fields[e]));
				}
