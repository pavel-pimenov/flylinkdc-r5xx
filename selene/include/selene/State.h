...
public:
    ...
    bool Load(std::string file_or_buffer, bool p_load_file, const char* p_name) {
        ResetStackOnScopeExit savedStack(_l);
		int status;
		if (p_load_file)
			status = luaL_loadfile(_l, file_or_buffer.c_str());
		else
		{
			status = luaL_loadbuffer(_l, file_or_buffer.c_str(), file_or_buffer.size(), p_name);
			file_or_buffer = p_name;
		}
        ...
        if (status != lua_ok) {
            if (status == LUA_ERRSYNTAX) {
                ...
                _exception_handler->Handle(status, msg ? msg : file_or_buffer + ": syntax error");
            } else if (status == LUA_ERRFILE) {
                const char *msg = lua_tostring(_l, -1);
                _exception_handler->Handle(status, msg ? msg : file_or_buffer + ": file error");
            }
            return false;
        }
        ...
        const char *msg = lua_tostring(_l, -1);
        _exception_handler->Handle(status, msg ? msg : file_or_buffer + ": dofile failed");
        return false;
    }
